...
public class AccountYingActivity extends Activity implements OnClickListener{
	...
	private static JSONObject getJSONByLotno(){
		JSONObject  jsonObjectByLotno =  RechargeDescribeInterface.getInstance().rechargeDescribe("bankChargeDescription");
...
//					Log.e("json.isNull===",""+json.isNull("creditCard"));
	...
	private void setAccountType(String name, AccountType bankType)throws JSONException {
		if(name.equals("zfb")){
    ...
    private void getAccountNet() {
	    ...
	    new Thread(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				try{
						 String re = AccountRechargeInterface.getInstance().recharge(userno,phonenum,TYPE);
						 ...
						 if(error_code.equals("0000")){
						    ...
						    handler.post(new Runnable() {
								...
								public void run() {
									// TODO Auto-generated method stub
									Toast.makeText(AccountYingActivity.this, message, Toast.LENGTH_SHORT).show();
								}
							});
						 }
					 }catch(JSONException e){
	...
	public void isTurnView(){
		RWSharedPreferences shellRW = new RWSharedPreferences(AccountYingActivity.this,ShellRWConstants.SHAREPREFERENCESNAME);
		...
		if(accountnumstr.length()>9){
			Toast.makeText(AccountYingActivity.this, "你有那么多钱吗?", Toast.LENGTH_SHORT).show();
			accountnum.setText("100");
			isOnClick = true;
		}else if(accountnumstr.equals("")){
			Toast.makeText(AccountYingActivity.this, "请输入充值金额!", Toast.LENGTH_SHORT).show();
			isOnClick = true;
		}else if(Integer.parseInt(accountnumstr)==0){
			Toast.makeText(AccountYingActivity.this, "充值金额不能为0！", Toast.LENGTH_SHORT).show();
			isOnClick = true;
		}else if(Integer.parseInt(accountnumstr)<20){
			Toast.makeText(AccountYingActivity.this, "充值金额至少为20元！", Toast.LENGTH_SHORT).show();
			isOnClick = true;
		}else{
			if(isAlipay){
	...
	private void getOrderInfo(final String userno,final String phonenum,final String amt) {
		// TODO Auto-generated method stub
			...
			public void run() {
				...
				try {
					...
					if(error_code.equals("0000")){
						...
						boolean bRet = msp.pay(info, mHandler, AlixId.RQF_PAY, AccountYingActivity.this);

						if (bRet) {
							// show the progress bar to indicate that we have started
							// paying.
							closeProgress();
							mProgress = BaseHelper.showProgress(AccountYingActivity.this, null, "正在支付", false,true);
						} 
					}else{
				...
				switch (msg.what) {
					case AlixId.RQF_PAY: {
						...
						try {
							...
							BaseHelper.showDialog(AccountYingActivity.this, "提示", memo,R.drawable.info);
							
						} catch (Exception e) {
							e.printStackTrace();
							BaseHelper.showDialog(AccountYingActivity.this, "提示", strRet,R.drawable.info);
						}
					}
						break;
					}
