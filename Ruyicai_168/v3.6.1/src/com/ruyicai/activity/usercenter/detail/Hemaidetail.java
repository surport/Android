...
public class Hemaidetail extends Activity implements HandlerMsg {
	...
	public void init() {
				...
				if (issharemove) {
					...
					anim.setAnimationListener(new AnimationListener() {
						...
						public void onAnimationEnd(Animation animation) {
							...
							lp.setMargins(PublicMethod.getPxInt(265, Hemaidetail.this), 0, 0, 0);
		...
		PublicMethod.createStar(starLayout, detatil.getCrown(), detatil.getGrayCrown(), detatil.getCup(),
				...
				detatil.getGrayStarNum(), Hemaidetail.this, 0);
	...
	public void tenoauth() {
		...
		if (tencent_token.equals("") && tencent_access_token_secret.equals("")) {
			...
			Intent intent = new Intent(Hemaidetail.this, OAuthV1AuthorizeWebView.class);// 创建Intent，使用WebView让用户授权
			...
			Intent intent = new Intent(Hemaidetail.this, TencentShareActivity.class);
	...
	public void setListViewHeightBasedOnChildren(ListView listView) {
		...
		params.height = PublicMethod.getPxInt(42, Hemaidetail.this) * (listAdapter.getCount());
	...
	public JoinDetatil getValue() {
		...
		try {
			...
			detatil.setWincode(json.getString("winCode"));
			detatil.setContentHtml(json.getString("contentHtml"));
			Constants.shareContent = "Hi，我刚使用了如意彩手机客户端买彩票，很方便呢！" + "你也试试吧，彩票随身投，大奖时时有！中奖了记的要请客啊！下载地址:"
					+ detatil.getUrl();
	...
	public void setValue(JoinDetatil detatil) {
		...
		content.setText(Html.fromHtml("方案内容：<br>"+detatil.getContentHtml()));
		amountProgress.setText("占总额" + progress(amountEdit.getText().toString(), detatil.getAtm()) + "%");
		safeProgress.setText("占总额" + progress(safeAmtEdit.getText().toString(), detatil.getAtm()) + "%");
	...
	public void getLoginInfo() {
		RWSharedPreferences shellRW = new RWSharedPreferences(Hemaidetail.this, "addInfo");
	...
	public void isJoinInNet() {
		...
		if (amount.equals("") && safeAmt.equals("")) {
			Toast.makeText(Hemaidetail.this, "认购金额或保底金额不能为空", Toast.LENGTH_SHORT).show();
		} else if (amountInt == 0 && safeAmtInt == 0) {
			Toast.makeText(Hemaidetail.this, "认购金额和保底金额不能都为零", Toast.LENGTH_SHORT).show();
		} else if (!isVisable) {
			joinInNet();
		} else if (safeAmtInt != 0 && amountInt == 0) {
			joinInNet();
		} else if (amountInt < Integer.parseInt(detatil.getMinAmt())) {
			Toast.makeText(Hemaidetail.this, "请您至少认购" + detatil.getMinAmt() + "元", Toast.LENGTH_SHORT).show();
		}
					...
					if (error.equals("0000")) {
						handler.post(new Runnable() {
							...
							public void run() {
								...
								Toast.makeText(Hemaidetail.this, msg, Toast.LENGTH_SHORT).show();
							}
						});
					} else {
						handler.post(new Runnable() {
