 ...
 * 
 */
public class GiftActivity extends TouzhuBaseActivity implements HandlerMsg ,OnSeekBarChangeListener{
	...
	public void init() {
		...
		}else if(Constants.type.equals("luck")){
			getNetIssue();
		}else{
		...
		ok.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				addPerson();
				RWSharedPreferences pre = new RWSharedPreferences(GiftActivity.this, "addInfo");
				sessionId = pre.getStringValue("sessionid");
				phonenum = pre.getStringValue("phonenum");
				userno = pre.getStringValue("userno");
				if(userno.equals("")){
					Intent intentSession = new Intent(GiftActivity.this, UserLogin.class);
...
//		setValue();
	...
	public void getTouzhuAlert(){
		if(Constants.type.equals("zc")){
			...
			jine.setText("金额："+iProgressQishu * (Integer.valueOf(betAndGift.getAmount())/100) * iProgressBeishu
					+ "元");
		}else{
		zhushu.setText(addview.getAllZhu() + "注     " );
		jine.setText("金额："+iProgressQishu * addview.getAllAmt() * iProgressBeishu
				+ "元");
		}
//		return "注数："
//				+ addview.getAllZhu() + "注     " 
...
//								}
				...
				for (int i = 0; i < successPersons.size(); i++) {
					String content = null;
			            if(betAndGift.getLotno().equals(Constants.LOTNO_JC)||betAndGift.getLotno().equals(Constants.LOTNO_JCLQ)
							...
							+ PublicMethod.toLotno(betAndGift.getLotno())+" 彩票"
							+ zhu + "注,"
							+ zhumaStr
							+ "留言如下："
							+ betAndGift.getAdvice()+" 请您登录如意彩平台领取彩票，详询www.ruyicai.com";
			            }else{
			            	content = "您的好友" + betAndGift.getPhonenum() + "送您"
	...
	public void giftNet() {
		...
		if(!Constants.type.equals("hight")&&!Constants.type.equals("zc")&&!Constants.type.equals("luck")){
