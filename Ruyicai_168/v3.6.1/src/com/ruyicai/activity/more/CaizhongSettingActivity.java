...
public class CaizhongSettingActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CaizhongSettingAdapter caizhongSettingAdapter = new CaizhongSettingAdapter(this, HomeActivity.shellRWList);
		caizhongSettListView.setAdapter(caizhongSettingAdapter);
		shellRW = new RWSharedPreferences(CaizhongSettingActivity.this, ShellRWConstants.CAIZHONGSETTING);
	}
	@Override
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if(keyCode == KeyEvent.KEYCODE_BACK) { //监控/拦截/屏蔽返回键
			for (int i = 0; i < HomeActivity.shellRWList.size(); i++) {
				if (shellRW.getStringValue(HomeActivity.shellRWList.get(i).get("shellKey").toString()).equals(Constants.CAIZHONG_OPEN)) {
					isNull = true;
				}
			}
	     }
		if (isNull) {
			...
			Toast.makeText(CaizhongSettingActivity.this, "请至少选择一项", Toast.LENGTH_SHORT).show();
			return true;
		}
	}
}