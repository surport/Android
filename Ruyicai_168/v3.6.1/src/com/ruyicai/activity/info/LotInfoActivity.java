...
public class LotInfoActivity extends Activity {
    ...
    private String[] expertId = {Constants.LOTNO_SSQ,Constants.LOTNO_DLT,Constants.LOTNO_FC3D,Constants.LOTNO_PL3,Constants.LOTNO_QLC,
    		                     Constants.LOTNO_22_5,Constants.LOTNO_PL5,Constants.LOTNO_QXC,Constants.LOTNO_SSQ};
...
//        view =mInflater.inflate(R.layout.information, mTabHost.getTabContentView(),true);
	 ...
	 */
 	 ...
 	 */
      public  class  TicketAdapter extends BaseAdapter{
         ...
         public TicketAdapter(Context context,List<LotInfoDomain> informationdata){
         	mInflater = LayoutInflater.from(context);
 			mList = informationdata;
         }
 		@Override
    ...
    private void getContent( final int type){
    	if(type == 1){
			...
			TicketAdapter adapter = new TicketAdapter(LotInfoActivity.this,informationdata);
	...
	private void getInfoNet(final int type,final String lotno) {
    	...
    	new Thread(new Runnable() {
			...
			@Override
			public void run() {
				String str = NewInformationInterface.getInstance().getInformationTitle(String.valueOf(type),lotno); //1彩民趣闻
				String strarry = null;
				try {
					JSONObject obj = new JSONObject(str);
