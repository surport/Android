...
public class TenActivity extends Dlc{
	protected String tenType[] = {"S1","H1","R2","R3","R4","R5","Q2","Q3","Z2","Z3"};//类型
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		initGroup();
		setTitleOne(getString(R.string.tenTitle));
		MobclickAgent.onEvent(this,"guangdongkuaileshifen" ); //BY贺思明 点击首页的“广东快乐十分”图标
	...
	public void initDTArea() {
			...
			areaNums[0] = new AreaNum(20,10, num-1, BallResId, 0, 1,Color.RED, "胆码");
	...
	public void initSpinner(){
			...
			adapter = ArrayAdapter.createFromResource(this, R.array.ten_type,android.R.layout.simple_spinner_item);// 实例化ArrayAdapter
	...
	public AreaNum[] initArea() {
		...
		if(state.equals("S1")){
			...
			areaNums[0] = new AreaNum(18,10,1, 18, BallResId, 0, 1,Color.RED, title,false,true);
		}else if(state.equals("H1")){
			String title = "请选择投注号码" ;
			areaNums = new AreaNum[1];
			areaNums[0] = new AreaNum(2,10,1, 2, BallResId, 0, 19,Color.RED, title,false,true);
		}else if(state.equals("Q2")){
			areaNums = new AreaNum[2];
			areaNums[0] = new AreaNum(20,10,1, 20, BallResId, 0, 1,Color.RED, wantitle,false,true,true);
			areaNums[1] = new AreaNum(20,10,1, 20, BallResId, 0, 1,Color.RED, qiantitle,false,true,true);
		}else if(state.equals("Q3")){
			areaNums = new AreaNum[3];
			areaNums[0] = new AreaNum(20,10,1, 20, BallResId, 0, 1,Color.RED, wantitle,false,true,true);
			areaNums[1] = new AreaNum(20,10,1, 20, BallResId, 0, 1,Color.RED, qiantitle,false,true,true);
			areaNums[2] = new AreaNum(20,10,1, 20, BallResId, 0, 1,Color.RED, baititle,false,true,true);
		}else if(state.equals("Z2")){
			areaNums = new AreaNum[1];
	        String title = "请选择投注号码" ;
			areaNums[0] = new AreaNum(20,10,2, 20, BallResId, 0, 1,Color.RED, title,false,true);
		}else if(state.equals("Z3")){
			areaNums = new AreaNum[1];
	        String title = "请选择投注号码" ;
			areaNums[0] = new AreaNum(20,10,3, 20, BallResId, 0, 1,Color.RED, title,false,true);
		}else{
			areaNums = new AreaNum[1];
	        String title = "请选择投注号码" ;
	        int isChoseNum = typeSpinner.getSelectedItemPosition();
			areaNums[0] = new AreaNum(20,10,isChoseNum, 20, BallResId, 0, 1,Color.RED, title,false,true);
		}
       return areaNums;
	}
...
//		isMissNet(new DlcMissJson(),sellWay,false);//获取遗漏值
	...
	public void createViewDT(int id){
		...
		createViewDanTuo(areaNums, sscCode,ZixuanAndJiXuan.NULL,true,id,false);
