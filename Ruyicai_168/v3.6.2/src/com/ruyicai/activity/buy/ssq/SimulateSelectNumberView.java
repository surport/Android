...
public class SimulateSelectNumberView extends View {
	...
	private void getViewBitmaps() {
		...
		// 获取目标尺寸的位图资源
		LotteryNumberCell.whiteGridBitmap = getBitmapFromResource(R.drawable.notice_center_wite, NumberCell.cellWidth,
				NumberRow.rowHight);
		LotteryNumberCell.grayGridBitmap = getBitmapFromResource(R.drawable.notice_center_grey, NumberCell.cellWidth,
				NumberRow.rowHight);
		LotteryNumberCell.redBallBitmap = getBitmapFromResource(R.drawable.notice_ball_red, NumberCell.cellWidth,
				NumberRow.rowHight);
		LotteryNumberCell.blueBallBitmap = getBitmapFromResource(R.drawable.notice_ball_blue, NumberCell.cellWidth,
				NumberRow.rowHight);
		BatchCodeCell.singleTitleBitmap = getBitmapFromResource(R.drawable.notice_left_red, BatchCodeCell.cellWidth,
				NumberRow.rowHight);
		BatchCodeCell.doubleTitleBitmap = getBitmapFromResource(R.drawable.notice_left_wite, BatchCodeCell.cellWidth,
				NumberRow.rowHight);
		BatchCodeCell.titleSelectBitmap = getBitmapFromResource(R.drawable.simulate_selectnumber_title,
				BatchCodeCell.cellWidth, SelectRow.rowHight);
		NumberCell.redSelectBitmap = getBitmapFromResource(R.drawable.simulate_selectnumber_bottomred,
				NumberCell.cellWidth, SelectRow.rowHight);
		NumberCell.blueSelectBitmap = getBitmapFromResource(R.drawable.simulate_selectnumber_bottomblue,
				NumberCell.cellWidth, SelectRow.rowHight);
		NumberCell.normalSelectBitmap = getBitmapFromResource(R.drawable.simulate_selectnumber_bottomnormal,
				NumberCell.cellWidth, SelectRow.rowHight);
	}

	private Bitmap getBitmapFromResource(int resourceId, int desWidth, int desHeight) {
	...
	private void setViewAttributes() {
		...
		Log.i(TAG, "NumberRow's rowhight =" + NumberRow.rowHight);

		SelectRow.rowHight = (int) ((int) 58 * ratio);
		Log.i(TAG, "SelectRow's rowhight =" + SelectRow.rowHight);

		BatchCodeCell.cellWidth = (int) ((int) 100 * ratio);
		Log.i(TAG, "BatchCodeCell's cellwidth =" + BatchCodeCell.cellWidth);

		NumberCell.cellWidth = (int) ((int) 58 * ratio);
		Log.i(TAG, "NumberCell's cellwidth =" + NumberCell.cellWidth);
	}
	...
	private int measuredWidth(int measureSpec) {
		...
		if (specMode == MeasureSpec.EXACTLY) {
			...
			result = BatchCodeCell.cellWidth + (Row.columNum - 1) * NumberCell.cellWidth;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			int touchedRow = caculateTouchRow(touchY);
			int touchedColum = caculateTouchColum(touchX);
	...
	private void updateSelectNumberShow() {
		...
		if (selectedNumber.length() > 3) {
			styleBuilder.setSpan(new ForegroundColorSpan(Color.RED), 3, selectedNumber.length(),
					Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
		}

		// 如果有篮球，在设置篮球为蓝色
		if (redEndIndex != 0) {
			styleBuilder.setSpan(new ForegroundColorSpan(Color.BLUE), redEndIndex, selectedNumber.length(),
					Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
		}
		...
		public NumberRow(int whichRow) {
			...
			for (int colum_i = 0; colum_i < columNum; colum_i++) {
				...
				if (isBatchCodeCell(colum_i)) {
					...
					cell = new LotteryNumberCell(Cell.RED_BALL, whichRow, colum_i);
				} else {
					cell = new LotteryNumberCell(Cell.BLUE_BALL, whichRow, colum_i);
				}
		...
		public SelectRow(int row) {
			...
			for (int colum_i = 0; colum_i < columNum; colum_i++) {
				...
				if (isBatchCodeCell(colum_i)) {
					...
					cell = new NumberCell(Cell.BLUE_BALL, row, colum_i, colum_i - redColumNum);
				}
	...
	static class Cell {
		...
		protected float caculateAlignLeft() {
			...
			if (isBatchCodeCell(colum)) {
				...
				return (float) BatchCodeCell.cellWidth + (colum - 1) * NumberCell.cellWidth;
			}
		}
		...
		protected float caculateTextAlignTop(float top) {
			...
			if (whichRow == viewRowNum) {
				textBaseY = (SelectRow.rowHight - (SelectRow.rowHight - fontHeight) / 2 - fontMetrics.bottom) + top;
			} else {
				textBaseY = (NumberRow.rowHight - (NumberRow.rowHight - fontHeight) / 2 - fontMetrics.bottom) + top;
			}
	...
	static class LotteryNumberCell extends Cell {
		...
		public void onDrawBackground(Canvas canvas) {
			...
			if (row % 2 == 0) {
				...
				if (colum % 2 == 0) {
					...
					canvas.drawBitmap(whiteGridBitmap, alignLeft, alignTop, null);
				}
			} else {
				if (colum % 2 == 0) {
					canvas.drawBitmap(whiteGridBitmap, alignLeft, alignTop, null);
		...
		public void onDrawLossValue(Canvas canvas) {
			...
			paint.setColor(Color.BLACK);
			String lossString = formatNumberToTwoPlace(loss);
			canvas.drawText(lossString, textAlignLeft, textAlignTop, paint);
		}
