...
public class AccountDetailsActivity extends Activity {
	...
	private final	String  AMT="amt",MEMO="memo",PLATTIME="platTime",TTRANSACTIONTYPE = "ttransactionType",TEXTCOLOR = "textcolor",BLSIGN = "blsign";
		...
		public void handleMessage(Message msg) {
			...
			case 3:
				...
				if(pagepayindex==0){
					...
					initLinear(linearcharge,linearId[2],initLinearView(pagepayindex,payPages,paydatalist));  	
					}else{
			...
			case 4:
				...
				if(pagesendprizeindex==0){
					...
					initLinear(linearcharge,linearId[3],initLinearView(pagesendprizeindex,sendPrizePages,sendprizesdatalist));  	
					}else{
			...
			case 5:
				...
				if(pagewithdrawindex==0){
					...
					initLinear(linearcharge,linearId[4],initLinearView(pagewithdrawindex,withdrawPages,withdrawdatalist));  	
					}else{
...
//			  setAllPage(typeAllPage);
			  ...
			  for(int i=0;i<winprizejson.length();i++){
				try{
					...
					accountDetailInfo.setBlsign(winprizejson.getJSONObject(i).getString(BLSIGN));
	 ...
	 * @return  初始化之后的view
	 */
	private View  initLinearView( final int pageindex,int allpage,final List typelist){
	...
	private void initViewInfo(int page,final List list,View view,ListView listView){
		LinearLayout layout = (LinearLayout) view.findViewById(R.id.layout_account);
		...
		case 1:
			...
			textName.setText(R.string.account_details_name_chongzhi);
			textAmt.setText(chargeAmt+"元");
			chargePages = page;
			chargedatalist = list;
			break;
		case 2:
			listView.setPadding(0, 0, 0, PublicMethod.getPxInt(30, this));
			layout.setVisibility(View.VISIBLE);
			textName.setText(R.string.account_details_name_touzhu);
			textAmt.setText(payAmt+"元");
			payPages = page;
			paydatalist = list;
			break;
		case 3:
...
// 根据type获得适配器
	private AccountQueryAdapter  getadapter(){
		...
		case 2:
			...
			break;
		case 3:
			sendprizesdatadadapter =  new AccountQueryAdapter(this, sendprizesdatalist);
			listadapter = sendprizesdatadadapter;
			break;
		case 4:
			withdrawdataadadapter =new AccountQueryAdapter(this, withdrawdatalist);
