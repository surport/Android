...
public class FootHun {
	...
	public static String bqcType[] = {"3","1","0", BQC_33, BQC_31, BQC_30, BQC_13, BQC_11,BQC_10, BQC_03, BQC_01, BQC_00,
		    ZJQ_00, ZJQ_01, ZJQ_02, ZJQ_03, ZJQ_04, ZJQ_05, ZJQ_06, ZJQ_07,
		    BF_10, BF_20, BF_21, BF_30,BF_31, BF_32, BF_40, 
		    BF_41, BF_42, BF_50, BF_51, BF_52, BF_90,BF_00, BF_11,
		    BF_22, BF_33, BF_99, BF_01, BF_02, BF_12, BF_03,BF_13,
		    BF_23, BF_04, BF_14, BF_24, BF_05, BF_15, BF_25,BF_09 };
	public static String titleStrs[] = { "胜","平","负","胜胜", "胜平", "胜负", "平胜", "平平", "平负","负胜", "负平", "负负",
		    "0", "1","2","3","4","5","6","7+",
	...
	private String getPlayCode(Info info,String lotno,int start,int end) {
		...
		String everyCode = info.getDay() + "|" + info.getWeek() + "|" + info.getTeamId() + "|"+lotno+"|";
		...
		for (int i = 0; i < listInfo.size(); i++) {
			...
			if (info.onclikNum > 0) {
				try {
					...
					for (int j = 0; j < info.check.length; j++) {
						if (info.check[j].isChecked()) {
							getMaxOdd(j,Double.parseDouble(info.getVStrs()[j]),aa);
						}
					}
					double[] insertdouble = PublicMethod.getDoubleArrayNoZero(aa);
					oddsList.add(insertdouble);
