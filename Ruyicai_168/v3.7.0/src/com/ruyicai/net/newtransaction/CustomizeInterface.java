...
public class CustomizeInterface {
	...
	public  static String customizeNet(CustomizeInfo info){
		...
		try {
			...
			jsonProtocol.put(ProtocolManager.STARTER_USERNO, info.getStarterUserNo());
			jsonProtocol.put(ProtocolManager.TIMES, info.getTimes());
			jsonProtocol.put(ProtocolManager.JOIN_AMT, PublicMethod.toFen(info.getJoinAmt()));
			jsonProtocol.put(ProtocolManager.SAFE_AMT, PublicMethod.toFen(info.getSafeAmt()));
			jsonProtocol.put(ProtocolManager.JOIN_TYPE, info.getJoinType());
			jsonProtocol.put(ProtocolManager.MAX_AMT, PublicMethod.toFen(info.getMaxAmt()));
