...
public class NoticeBallView extends View {
	...
	case MotionEvent.ACTION_UP:
		...
		if(absX<30&&absY<30){
			float x = ev.getX();
			float y = ev.getY();
			if(textCode!=null&&!(iGameType.equalsIgnoreCase("qlc")&&!isRed)){
				for(int i=0;i<ballsList.size();i++){
					...
					if((x>ballX&&x<ballX+width)&&(y>ballY&&y<ballY+width)){
	...
	private boolean isBallChecked(float ballX, float ballY) {
		...
		for(int j=0;j<ballsChcek.size();j++){
			if(ballsChcek.get(j).getLeft()==ballX&&ballsChcek.get(j).getTop()==ballY){
				ballsChcek.remove(j);
				isOnclick = false;
				break;
			}else if(ballsChcek.get(j).getLeft()==ballX){
				if (toast == null) {
					toast = Toast.makeText(context, "已经选择了这个号码", Toast.LENGTH_LONG);
					toast.show();
				} else {
	...
	public void initImage() {
		...
		bitBlackBall = getBitmapFromRes(R.drawable.notice_ball_black, WITH, WITH);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if(textCode!=null){
			...
			for(int i=0;i<ballsChcek.size();i++){
				onDrawBallOnclik(canvas,ballsChcek.get(i).getLeft(), ballsChcek.get(i).getTop(),ballsChcek.get(i).getNum());
			...
			if(isRed){
				...
				PublicMethod.setTextColor(textCode, "|"+codeStr, 0, 1, Color.BLACK);
			}
	 ...
	 * @return
	 */
	public static ArrayList<BallPosition> rankList(ArrayList<BallPosition> rankInt) {
		// 取长度最长的词组 -- 冒泡法
	...
	public void onDrawLeft(Canvas canvas) {
		...
		for (int i = 0; i < line; i++) {
			...
			canvas.drawText(list.get(i).getIssue(), with, WITH + i
					* WITH + height, p);
			if(isRed){
			...
			if (list.get(i).getBallNum() != null) {
				for (int j = 0; j < list.get(i).getBallNum().length; j++) {
					...
					if(isTen){
						isNumber = PublicMethod.isTen(list.get(i).getBallNum()[j]);
					}else{
						isNumber = Integer.toString(list.get(i).getBallNum()[j]);
					}
					...
					//篮球
					if (j == list.get(i).getBallNum().length - 1&& iGameType.equals("cjdlt")) {
						p.setColor(0xFF073AAC);
						canvas.drawText(","+ PublicMethod.isTen(blueList.get(i).getBallNum()[1]),FIRST_WITH - SECOND_WITH - toLeft + SECOND_WITH/ sum * (j + 2) + 4, WITH + i * WITH+ height, p);
						p.setColor(0xFFDE0201);
					}
					
					if (j == list.get(i).getBallNum().length - 1&& (iGameType.equals("ssq")|| iGameType.equals("qlc") || iGameType.equals("cjdlt"))) {
						p.setColor(0xFF073AAC);
						canvas.drawText(","+ PublicMethod.isTen(blueList.get(i).getBallNum()[0]),FIRST_WITH - SECOND_WITH - toLeft + SECOND_WITH/ sum * (j + 1) + 4, WITH + i * WITH+ height, p);
						p.setColor(0xFFDE0201);
					}
			...
			if(iGameType.equals("fc3d")){
				if(list.get(i).getTrycode() != null){
					for (int j = 0; j < list.get(i).getTrycode().length; j++) {
						...
						if(isTen){
							isNumber = PublicMethod.isTen(list.get(i).getTrycode()[j]);
						}else{
							isNumber = Integer.toString(list.get(i).getTrycode()[j]);
						}
						if (j != list.get(i).getBallNum().length - 1) {
							isNumber += ",";
						} 
						canvas.drawText(isNumber, FIRST_WITH- SECOND_WITH/2 + toLeft/2 + (SECOND_WITH /sum) * j, WITH + i * WITH + height, p);
				}				
			}
		   }
		}
//多画两行空白
		if (line % 2 == 0) {
			...
			canvas.drawBitmap(bitFirstLeftRed, 0, WITH + (line+1) * WITH, null);
		} else {
		...
		if(iGameType.equals("fc3d")){
		canvas.drawLine(FIRST_WITH - SECOND_WITH/2 - toLeft/4, 0, FIRST_WITH
					- SECOND_WITH/2 - toLeft/4, getHeight(), p);
		}
		canvas.drawLine(FIRST_WITH, 0, FIRST_WITH, getHeight(), p);
	}
			...
			for (int j = 0; j < row; j++) {
				...
				for (int n = 0; n < balls.length; n++) {
					if (num == balls[n]) {
						...
						BallPosition ball = new BallPosition(FIRST_WITH + j * WITH,WITH + i * WITH,WITH,PublicMethod.isTen(balls[n]));
						ballsList.add(ball);
						p.setColor(Color.WHITE);
