...
public class JoinDetailActivity extends Activity implements HandlerMsg {
	...
	public void init() {
		...
		amountEdit.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				isAmountZero = amount;
				String renAmt = leavMount(detatil.getRemainderAmt(), amountEdit.getText().toString());
		...
		safeAmtEdit.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				safeProgress.setText("占总额" + progress(isNull(amt), detatil.getTotalAmt()) + "%");
	...
	public JoinDetatil getValue() {
		...
		try {
			...
			detatil.setParticipantCount(jsonObject.getString("participantCount"));
	...
	public void setValue(JoinDetatil detatil) {
		...
		progress.append(detatil.getBuyProgress() + "%+" + detatil.getSafeProgress()+"%");
...
//		content.append(detatil.getContent());
		contentListView.createListContent(layoutMain,content,detatil.getLotNo(),detatil.getBetCodeHtml(),detatil.getBetCodeJson());
		...
		amountProgress.setText("占总额" + progress(amountEdit.getText().toString(), detatil.getTotalAmt()) + "%");
		safeProgress.setText("占总额" + progress(safeAmtEdit.getText().toString(), detatil.getTotalAmt()) + "%");
		showMinText(detatil.getMinAmt());
		leavTextView(amountText, true);
		leavTextView(safeText, false);
		PublicMethod.createStar(starLayout, String.valueOf(detatil.getCrown()), String.valueOf(detatil.getGraycrown()), String.valueOf(detatil.getCup()),
				String.valueOf(detatil.getGraycup()), String.valueOf(detatil.getDiamond()), String.valueOf(detatil.getGraydiamond()), String.valueOf(detatil.getGoldStar()),
				String.valueOf(detatil.getGraygoldStar()), JoinDetailActivity.this, 0);
	...
	public void joinDetailNet() {
		...
		Thread t = new Thread(new Runnable() {
			...
			@Override
			public void run() {
				str = QueryJoinDetailInterface.queryLotJoinDetail(caseId, userno);
				try {
					json = new JSONObject(str);
	...
	class JoinDetatil {
		...
		public void setBuyAmtByStarter(String buyAmtByStarter) {
			this.buyAmtByStarter = String.valueOf(Integer.valueOf(buyAmtByStarter)/100);
		}
		...
		public void setRemainderAmt(String remainderAmt) {
			this.remainderAmt = String.valueOf(Integer.valueOf(remainderAmt)/100);
		}
