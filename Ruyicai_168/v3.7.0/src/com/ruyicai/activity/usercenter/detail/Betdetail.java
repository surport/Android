...
//		content.setText(Html.fromHtml("方案内容：<br>"+betcodehtml));
		...
		if(lotno.equals("J00001")||lotno.equals("J00002")||lotno.equals("J00003")||lotno.equals("J00004")||lotno.equals(Constants.LOTNO_JCLQ)
				||lotno.equals(Constants.LOTNO_JCLQ_DXF)||lotno.equals(Constants.LOTNO_JCLQ_RF)||lotno.equals(Constants.LOTNO_JCLQ_SFC)
				||lotno.equals(Constants.LOTNO_JCZQ_HUN)||lotno.equals(Constants.LOTNO_JCLQ_HUN)){
		...
		public void onClick(View v) {
			...
			Toast.makeText(Betdetail.this, "请输入数字", Toast.LENGTH_SHORT).show();	
				...
				if(allAmount>0){
					...
					Toast.makeText(Betdetail.this, getString(R.string.usercenter_max_amount), Toast.LENGTH_SHORT).show();
				}
	...
	private void getNetIssue( final String lotno,final String lotname,final int allAmount,final String lommt,final String betcode) {
		showDialog(0);
		new Thread(new Runnable() {
			...
			if(issue.equals("")){
	            handler.post(new Runnable() {
					...
					public void run() {
						dismissDialog(0);
					Toast.makeText(Betdetail.this, "获取期号失败", Toast.LENGTH_SHORT).show();
					}
				});
			}else{
				handler.post(new Runnable() {
			...
			public void run() {
				...
				try {
				    ...
				    handler.post(new Runnable() {
						...
						public void run() {
						dismissDialog(0);
						Toast.makeText(Betdetail.this, message, Toast.LENGTH_SHORT).show();
						}
					});
				} catch (JSONException e) {
					e.printStackTrace();
