...
public class ClockThread {
	private static String[] times = new String[11];
	public static String[] lotnos = { Constants.LOTNO_SSQ, Constants.LOTNO_DLT,
			Constants.LOTNO_FC3D, Constants.LOTNO_SFC, Constants.LOTNO_PL3,
			Constants.LOTNO_PL5, Constants.LOTNO_QXC, Constants.LOTNO_QLC,
			Constants.LOTNO_JQC, Constants.LOTNO_RX9, Constants.LOTNO_LCB };
	...
	public static String formatTime(String times) {
		if (times != null) {
			if (Integer.parseInt(times) < 0) {
				...
				day = time / (3600 * 24);
				return day + "天" + df.format(hour) + "时" + df.format(minute)
						+ "分" + df.format(second) + "秒";
			}
		} else {
			return "0天00时00分00秒";
		}
