...
public class ExplainListActivity extends BaseListActivity {
	...
	protected List initExplainInfo(){
		try {
			JSONObject jsonObject  = JcExplainActivity.jsonObject.getJSONObject("result").getJSONObject("schedule");
			homeTeamId = jsonObject.getString("homeTeamId");
			guestTeamId = jsonObject.getString("guestTeamId");
		} catch (JSONException e) {
			e.printStackTrace();
		}
...
//				holder.btn.setVisibility(Button.GONE);
	...
	private View createRank(ExplainInfo info,boolean isTop){
		...
		if(isTop){
			...
			SpannableStringBuilder team =new SpannableStringBuilder(info.getTeamName());
			if (info.getTeamId().equals(homeTeamId)) {
				team.setSpan(new ForegroundColorSpan(Color.RED), 0, team.length(),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE ); 
			}else if(info.getTeamId().equals(guestTeamId)){
				team.setSpan(new ForegroundColorSpan(Color.BLUE), 0, team.length(),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE );
			}
	...
	private View createTerm10(ExplainInfo info,boolean isTop){
		...
		if(isTop){
			...
			SpannableStringBuilder home =new SpannableStringBuilder(info.getHomeTeam());
			if (info.getHomeTeamId().equals(homeTeamId)) {
				home.setSpan(new ForegroundColorSpan(Color.RED), 0, home.length(),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE ); 
			}else if(info.getHomeTeamId().equals(guestTeamId)){
				home.setSpan(new ForegroundColorSpan(Color.BLUE), 0, home.length(),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE );
			}
			
			SpannableStringBuilder away =new SpannableStringBuilder(info.getGuestTeam());
			if (info.getGuestTeamId().equals(homeTeamId)){
				away.setSpan(new ForegroundColorSpan(Color.RED), 0, away.length(),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE ); 
			}else if (info.getGuestTeamId().equals(guestTeamId)) {
				away.setSpan(new ForegroundColorSpan(Color.BLUE), 0, away.length(),Spannable.SPAN_EXCLUSIVE_EXCLUSIVE ); 
			}
