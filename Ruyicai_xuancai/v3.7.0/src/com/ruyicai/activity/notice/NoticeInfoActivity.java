...
public class NoticeInfoActivity extends Activity{
	...
	private final int LISTSSQ = 0,LIST3D = 1,LISTQLC = 2,LISTPL3 = 3,LISTDLT = 4,LISTSSC = 5
					...
					,LISTPL5 = 11,LISTQXC = 12,LISTYDJ = 13,LISTTWENTY = 14,LISTTEN = 15,ZC = 16;
...
//		if (NoticeMainActivity.isFirstNotice) {
	...
	public BaseAdapter getAdapter(String lotno){
		if(lotno.equals(Constants.LOTNO_SSC)||lotno.equals(Constants.LOTNO_11_5)||
				lotno.equals(Constants.LOTNO_eleven)||lotno.equals(Constants.LOTNO_GD115)||lotno.equals(Constants.LOTNO_ten)){
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(lotType==LIST3D){
				...
				for (i1 = 0; i1 < 3; i1++) {
					...
					tempBallView.initBall(NoticeMainActivity.BALL_WIDTH, NoticeMainActivity.BALL_WIDTH, ""+ iShowNumber, aRedColorResId);
				...
				for (i1 = 0; i1 < 3; i1++) {
					codeshow  += trycodestr.substring(i1 * 2+1,
							i1 * 2 + 2);
					if(i1!=2){
						codeshow+=",";	
					}				    
				...
				public void onClick(View v) {
					...
					if(isImg){
						...
						case LISTTWENTY:
							...
							break;
						case ZC:
							ZCDetailView zcview = new ZCDetailView(context, Lotno, iIssueNo, progress,new Handler(),true);
							break;
						}
					}
		...
		super.onActivityResult(requestCode, resultCode, data);
		 if (requestCode==1)	{
				if (resultCode==OAuthV1AuthorizeWebView.RESULT_CODE)	{
					...
					Intent intent=new Intent(NoticeInfoActivity.this, TencentShareActivity.class);
