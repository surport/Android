...
/**
 * 用来存取AccessToken和SesionKey；开发不会直接使用该类
 * 
...
class AccessTokenManager implements Parcelable {
	...
	public boolean isSessionKeyValid() {
		...
		//尝试从SharedPreference中对sessionkey进行初始�?
		initSessionKey();
		if(sessionKey != null && sessionSecret != null 
				&& System.currentTimeMillis() < expireTime) {
			return true;
	...
	void restoreSessionKey() {
		...
		try {
			initSessionKey();
			//若没有session Key或�?Session key已经过期
	...
	/**
	 * 当前的session信息与配置文件是否一直，若不，则对当前的sessionkey等进行初始化
	 * @return
	 */
	private void compareWithConfig() {
		...
		// 对配置的变化进行监听，使得有多个Renren对象时，可以对权限信息保持一�?
	...
	private void exchangeSessionKey(String accessToken) {
		...
		try {
			...
			// 存储当前获得的sessionKeysessionSecret
