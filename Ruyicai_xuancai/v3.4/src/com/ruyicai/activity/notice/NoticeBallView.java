			...
			if (list.get(line - 1 - i).getBallNum() != null) {
				for (int j = 0; j < list.get(line - 1 - i).getBallNum().length; j++) {
					...
					if (j == list.get(line - 1 - i).getBallNum().length - 1
						...
						canvas.drawText(
								","
										+ PublicMethod.isTen(blueList.get(
												line - 1 - i).getBallNum()[1]),
								FIRST_WITH - SECOND_WITH - toLeft + SECOND_WITH
					...
					if (j != list.get(line - 1 - i).getBallNum().length - 1) {
						canvas.drawText(
								""
										+ PublicMethod.isTen(list.get(
												line - 1 - i).getBallNum()[j])
										+ ",", FIRST_WITH - SECOND_WITH
										- toLeft + SECOND_WITH / sum * j + 4,
								WITH + i * WITH + height, p);
					} else {
						canvas.drawText(PublicMethod.isTen(list.get(
								line - 1 - i).getBallNum()[j]), FIRST_WITH
								- SECOND_WITH - toLeft + SECOND_WITH / sum * j
								+ 4, WITH + i * WITH + height, p);
					}
					if (j == list.get(line - 1 - i).getBallNum().length - 1
