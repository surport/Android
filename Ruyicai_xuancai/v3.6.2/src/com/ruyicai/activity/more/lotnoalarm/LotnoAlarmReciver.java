...
public class LotnoAlarmReciver extends BroadcastReceiver {
	...
	public void startServiceInFiexdTime(Context context) {
		...
		Intent intentForAlarmService = new Intent(context, LotnoAlarmService.class);
		PendingIntent pendingIntentForAlarmService = PendingIntent.getService(context, 0, intentForAlarmService, 0);
		...
		alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, calendar.getTimeInMillis(), MILLISCEONDS_OF_ONEDAY,
	...
	private Calendar caculateAlarmTime(Context context) {
		...
		int setHour = lotnoAlarmManager.getAlarmTimeSetting(LotnoAlarmManager.PREFERENCE_HOUR_KEY);
		int setMinute = lotnoAlarmManager.getAlarmTimeSetting(LotnoAlarmManager.PREFERENCE_MINUTE_KEY);
