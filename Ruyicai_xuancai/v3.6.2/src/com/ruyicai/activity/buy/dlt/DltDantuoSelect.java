...
public class DltDantuoSelect extends ZixuanActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		getMissNet(new DltMissJson(),MissConstant.DLT_Miss,MissConstant.DLT_DAN);//联网获取遗漏值
	}
	/**
	 * 初始化滑动
	 */
	...
	public AreaNum[] initArea() {
        ...
        areaNums[0] = new AreaNum(35,8,1, 4, redBallResId, 0, 1,Color.RED,redTitle,true,false);
        areaNums[1] = new AreaNum(35,8,2, 24, redBallResId, 0, 1,Color.RED,redtuoTitle,true,false);
        areaNums[2] = new AreaNum(12,8,1, 1, blueBallResId, 0, 1,Color.BLUE,blueTitle,true,false);
        areaNums[3] = new AreaNum(12,8,2,12, blueBallResId, 0, 1,Color.BLUE,bluetuoTitle,true,false);
        return areaNums;
	}
				...
				for(int j=0;j<zhumaStr.length;j++){
				  ...
				  for(int i=0;i<zhuma.size();i++){
				     ...
				     if(i!=zhuma.size()-1){	
					  ...
					  builder.setSpan(new ForegroundColorSpan(itemViewArray.get(0).areaNums[i].textColor), length-zhuma.get(i).length(), length, Spanned.SPAN_COMPOSING);  
