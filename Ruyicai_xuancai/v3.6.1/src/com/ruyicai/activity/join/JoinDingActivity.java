...
import com.palmdream.RuyicaiAndroidxuancai.R.id;
...
public class JoinDingActivity extends Activity {
	 ...
	 * 初始化发起人信息
	 */
	private void initInfoView(String name, String crown, String grayCrown, String cup, String grayCup, String diamond,
			String grayDiamond, String star, String grayStar, String lotno, String person) {
		...
		PublicMethod.createStar(starLayout, crown, grayCrown, cup, grayCup, diamond, grayDiamond, star, grayStar, this,
				0);
	...
	public void setValueThread(final EditText mTextBeishu, final Handler handler, final int minInt) {
	...
	protected void infoNet(final String starterUserNo, final String lotno) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				try {
					String re = CustomizeInfoInterface.getInstance().customizeNet(starterUserNo, lotno);
	...
	private void jsonToStr(final String lotno, final JSONObject obj) {
		...
		try {
			...
			initInfoView(name, crown, grayCrown, cup, grayCrown, diamond, grayDiamon, star, grayStar, lotno, person);
		} catch (Exception e) {
	...
	protected boolean initPojo() {
		...
		if (customizeInfo.isAmt()) {
			if (amtEdit.getText().toString().equals("") || numEdit.getText().toString().equals("")) {
			...
			if (percentEdit.getText().toString().equals("") || percentNumEdit.getText().toString().equals("")) {
				Toast.makeText(context, "输入框不能为空", Toast.LENGTH_SHORT).show();
				isNull = true;
			} else {
