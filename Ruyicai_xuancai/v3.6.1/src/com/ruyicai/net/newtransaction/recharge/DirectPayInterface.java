...
public class DirectPayInterface {
	...
	public String directPay(BetAndGiftPojo betAndGift, boolean isAlipaySecure) {
		...
		try {
			...
			jsonProtocol.put(ProtocolManager.PHONENUM, betAndGift.getPhonenum());
			jsonProtocol.put(ProtocolManager.BETCODE, betAndGift.getBet_code());
			jsonProtocol.put(ProtocolManager.BATCHCODE, betAndGift.getBatchcode());
			jsonProtocol.put(ProtocolManager.LOTNO, betAndGift.getLotno());
			jsonProtocol.put(ProtocolManager.LOTMULTI, betAndGift.getLotmulti());
			jsonProtocol.put(ProtocolManager.AMOUNT, betAndGift.getAmount());
			if (betAndGift.getAmt() != 0) {
				jsonProtocol.put(ProtocolManager.ONEAMOUNT, betAndGift.getAmt() + "00");
			} else {
				jsonProtocol.put(ProtocolManager.ONEAMOUNT, betAndGift.getOneAmount());

			}
			
			jsonProtocol.put(ProtocolManager.ISSELLWAYS, betAndGift.getIsSellWays());
			if (isAlipaySecure) {
