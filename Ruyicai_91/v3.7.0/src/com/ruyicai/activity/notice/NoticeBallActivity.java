...
import com.ruyicai.activity.notice.NoticeBallView.BallPosition;
...
public class NoticeBallActivity extends Activity{
	...
	private void setTouZhuInfo(List<Integer> redList, List<Integer> blueList){
		...
		case NoticeActivityGroup.ID_SUB_SHUANGSEQIU_LISTVIEW:
			redNum = 6;blueNum =1;
			if (isBetLegitimacy(redList.size(), blueList.size(),redNum,blueNum)) {
				lotno = Constants.LOTNO_SSQ;
				code = SsqZiZhiXuanCode.simulateZhuma(redList, blueList);
				betNums = caculateBetNums(redList.size(), blueList.size(),redNum,blueNum);
	...
	private boolean isBetLegitimacy(int redSize, int blueSize,int redNum,int blueNum) {
	...
	private void addViewAndTouZhu(long betNums,List<Integer> reds, List<Integer> blues,String lotno,String code) {
	...
	private long caculateBetNums(int redSize, int blueSize,int redNum,int blueNum) {
		long betNums = PublicMethod.zuhe(redNum, redSize) * PublicMethod.zuhe(blueNum, blueSize);
		return betNums;
	}
