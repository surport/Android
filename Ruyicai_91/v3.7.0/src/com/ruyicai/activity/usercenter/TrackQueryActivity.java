...
public class TrackQueryActivity extends Activity implements HandlerMsg{
	...
	final	String BETCODE="betCode",BATCHNUM="batchNum",ORDERTIME="orderTime",ID = "id",
						LOTNO="lotNo",LOTNAME = "lotName",AMOUNT="amount",LASTNUM = "lastNum",BEGINBATCH = "beginBatch"
						 ...
						 BET_CODE="bet_code",LOTMULTI="lotMulti",ONEAMOUNT = "oneAmount",BETNUM="betNum";
		  ...
		  try {
			  ...
			  for(int i=0;i<winprizejson.length();i++){
				try{
					...
					winPrizeQueryinfo.setRemainderAmount(winprizejson.getJSONObject(i).getString("remainderAmount"));
		...
		private LayoutInflater mInflater; // 扩充主列表布局
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				holder.lookdetail.setOnClickListener(new OnClickListener() {
					public void onClick(View v) {
						  Intent intent = new Intent(TrackQueryActivity.this, Trackdetail.class);
	...
	public void createContinueDialog(final TrackQueryInfo info){
			...
			RelativeLayout relativeLayout = (RelativeLayout) viewDialog.findViewById(R.id.linearlayout_usercenter_beishu);
			final EditText editIssue = (EditText) viewDialog.findViewById(R.id.usercenter_edittext_bindphoneContext);
			final EditText editBeishu = (EditText) viewDialog.findViewById(R.id.usercenter_edittext_beishuContext);
...
//			contentTitle.setTextSize(PublicMethod.getPxInt(14, context));
			...
			ok.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
				   ...
				   if(issueNum.equals("")){
					   ...
					   Toast.makeText(context, "您输入的期数第一位不能为零！", Toast.LENGTH_SHORT).show();
				   }else if(issueNum.length()>5||Integer.parseInt(issueNum)>200){
					   Toast.makeText(context, "您输入的期数不能大于200期！", Toast.LENGTH_SHORT).show();
				   }else if(Integer.parseInt(issueNum)==0){
					   Toast.makeText(context, "您输入的期数不能为零！", Toast.LENGTH_SHORT).show();
				   }else if(beishu.equals("")){
					   Toast.makeText(context, "您输入的倍数不能为空！", Toast.LENGTH_SHORT).show();
				   }else if(Integer.parseInt(beishu.substring(0, 1))==0){
					   Toast.makeText(context, "您输入的期数第一位不能为零！", Toast.LENGTH_SHORT).show();
				   }else if(beishu.length()>5||Integer.parseInt(beishu)>2000){
					   Toast.makeText(context, "您输入的倍数不能大于2000倍！", Toast.LENGTH_SHORT).show();
				   }else if(Integer.parseInt(beishu)==0){
					   Toast.makeText(context, "您输入的倍数不能为零！", Toast.LENGTH_SHORT).show();
				   }else{
					   int oneAmt = Integer.parseInt(info.getOneAmount())/100*Integer.parseInt(info.getBetNum())*Integer.parseInt(beishu);
					   long amount = oneAmt*PublicMethod.toInt(issueNum);
					   if(amount > 0){
						   getIssue(info,issueNum,beishu);
						   continueDialog.cancel();	
					   }else{
						   Toast.makeText(TrackQueryActivity.this, getString(R.string.usercenter_max_amount), Toast.LENGTH_SHORT).show();
					   }
	 ...
	 */
	public void getIssue(final TrackQueryInfo info,final String issueNum,final Handler updateIssueHandler,final String beishu){
...
//					JSONObject temp_obj = Constants.currentLotnoInfo.getJSONObject(info.getLotno());
	...
	private void initBetPojo(String zhuma,String issue,String lotMulti,String lotno,String amount,String oneAmount) {
		initPojo();
	 ...
	 * 继续追号对话框
	 */
	public void createContinueInfo(TrackQueryInfo info,String issueNum,String issue,String beishu){
		int oneAmt = Integer.parseInt(info.getOneAmount())/100*Integer.parseInt(info.getBetNum())*Integer.parseInt(beishu);
		...
		initBetPojo(info.getBetTouCode(),issueNum,beishu,info.getLotno(),oneAmt+"00",info.getOneAmount());//初始化投注信息
		continueInfoDialog("\n"+continueStr,issue);
	}
