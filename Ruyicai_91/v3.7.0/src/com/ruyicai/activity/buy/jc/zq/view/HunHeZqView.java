...
public class HunHeZqView extends JcMainView{
	...
	public void setDifferValue(JSONObject jsonItem,Info itemInfo) throws JSONException{
		...
		for(int j=0;j<bqc_lenght;j++){
			itemInfo.getVStrs()[j+spf_lenght] = jsonItem.getString("half_v"+FootHun.bqcType[j+spf_lenght]);
		}
		for(int j=0;j<zjq_lenght;j++){
			itemInfo.getVStrs()[j+spf_lenght+bqc_lenght] = jsonItem.getString("goal_v"+j);
		}
		for(int j=0;j<bf_lenght;j++){
			itemInfo.getVStrs()[j+spf_lenght+bqc_lenght+zjq_lenght] = jsonItem.getString("score_v"+FootHun.bqcType[j+spf_lenght+bqc_lenght+zjq_lenght]);
		}
		initTitles(itemInfo);
	}
			...
			btn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(info.onclikNum>0||isCheckTeam()){
						info.createDialog(FootHun.titleStrs,true,info.getHome()+" VS "+info.getAway());
					}
					isNoDan(info,btnDan);
				}
			});
