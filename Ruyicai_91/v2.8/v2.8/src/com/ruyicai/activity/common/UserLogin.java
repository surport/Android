...
//		});
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				if (error_code.equals("0000")) {
					...
					shellRW.setUserLoginInfo("name", name);
					shellRW.setUserLoginInfo("userno", userno);
					shellRW.setUserLoginInfo("phonenum", phonenum);
					shellRW.setUserLoginInfo("mobileid", mobileid);//向ShellRWSharesPreferences中写入绑定手机号
	...
	private void turnToReg() {
		...
		final EditText register_recommand_num = (EditText) findViewById(R.id.register_recommender_num_edit);//91
        ...
        register_button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String register_recommand_phonenum = register_recommand_num.getText().toString();
	            ...
	            if(register_recommand_phonenum!=null&&register_recommand_phonenum!=""&&register_recommand_phonenum.length()!=0){
					
					if(register_recommand_phonenum.length() != 11){
						Toast.makeText(getBaseContext(), "手机号必须为11位！",Toast.LENGTH_SHORT).show();
					}
				}
				if (phoneNum.length() != 11) {
					...
					if(cardId.equals("")||cardId == null){
						...
						beginRegister(phoneNum,password,cardId,realName,isBindPhone,register_recommand_phonenum);
					}
				}else{
					beginRegister(phoneNum,password,cardId,realName,isBindPhone,register_recommand_phonenum);
				}
	 ...
	 * 注册联网操作
	 */
	private void beginRegister(final String phoneNum,final String partPassword,final String cardId,final String name,final String isBindPhone,final String recommand_phonenum) {
		showDialog(PROGRESS_VALUE);
		Thread regthread = new Thread(new Runnable() {
			...
			public void run() {
				...
				String re = RegisterInterface.getInstance().userregister(phoneNum, partPassword, cardId, name,isBindPhone,recommand_phonenum);
				JSONObject obj;
				String error_code = "0";
				try {
