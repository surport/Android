...
public class Renren implements Parcelable {
	...
	public Renren(Parcel in) {
		...
		Constants.apiKey = bundle.getString(KEY_API_KEY);
		Constants.secret = bundle.getString(KEY_SECRET);
		Constants.appId = bundle.getString(KEY_APP_ID);
	...
	private void authorize(Activity activity, String[] permissions, final RenrenDialogListener listener, String redirectUrl, String responseType) {
		...
		params.putString("client_id", Constants.apiKey);
	...
	public void authorize(Activity activity, PasswordFlowRequestParam param, final RenrenAuthListener listener) {
		...
		param.setApiKey(Constants.apiKey);
		param.setSecretKey(Constants.secret);
	...
	private void dialog(Context context, final RenrenDialogListener listener,
		...
		params.putString("api_key", Constants.apiKey);
	...
	public void writeToParcel(Parcel dest, int flags) {
		...
		if (Constants.apiKey != null) {
			bundle.putString(KEY_API_KEY, Constants.apiKey);
		}
		if (Constants.secret != null) {
			bundle.putString(KEY_SECRET, Constants.secret);
		}
		if (Constants.appId != null) {
			bundle.putString(KEY_APP_ID, Constants.appId);
		}
