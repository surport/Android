...
public class BindIDActivity extends Activity {
	...
	public void editPassword() {
		...
		final String nameStr = realnameEdit.getText().toString();
		final String cerdidStr = cerdidEdit.getText().toString();
		...
		if (nameStr.length() > 1&& cerdidStr.length() >1) {
				...
				new Thread(new Runnable() {
					public void run() {
						String bindIDback = BindIDInterface.getInstance().bindID(phonenum, cerdidStr,sessionid,userno);
							try {
