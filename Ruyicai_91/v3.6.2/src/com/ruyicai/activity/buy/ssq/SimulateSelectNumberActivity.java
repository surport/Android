...
public class SimulateSelectNumberActivity extends Activity implements OnClickListener {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GET_LOSSVALUE_ERROR:
				Toast.makeText(SimulateSelectNumberActivity.this, "遗漏值信息获取失败..." + msgString, Toast.LENGTH_LONG).show();
				break;
			...
			case GET_PRIZEINFO_ERROR:
				Toast.makeText(SimulateSelectNumberActivity.this, "历史获奖信息获取失败..." + msgString, Toast.LENGTH_LONG)
						.show();
				break;
	...
	private void initScreenShow() {
		...
		selectedNumbersTextView.setText("已选：");
	...
	private JSONObject getLossValueFromIntent() {
		PrizeInfoInterface prizeInfoInterface = PrizeInfoInterface.getInstance();
		JSONObject prizeInfoJsonObject = prizeInfoInterface.getNoticePrizeInfo(Constants.LOTNO_SSQ,
				String.valueOf(pageIndex), String.valueOf(maxResult));

		return prizeInfoJsonObject;
	}

	private JSONObject getPrizeInfoFromInternet() {
		QueryLossValueInterface queryLossValueInterface = QueryLossValueInterface.getInstance();
		JSONObject lossValueJsonObject = queryLossValueInterface.lossValueQuery(sellWay, lotNo,
	...
	private void analyzeJsonPrizeInfo(JSONObject prizeInfoJsonObject) {
		try {
			...
			// 网络获取成功，解析网络
				...
				JSONArray prizeInfoArray = prizeInfoJsonObject.getJSONArray("result");
				...
				for (int i = 0; i < length; i++) {
					...
					// Log.i(TAG, "winCode " + i + " =" + winCode);

					PrizeInfo prizeInfo = new PrizeInfo(batchCode, winCode, openTime);
	...
	private void analyzeJsonLossValue(JSONObject lossValueJsonObject) {
		try {
			...
			// 网络获取成功,解析Json数据
				...
				JSONArray lossValueArray = lossValueJsonObject.getJSONArray("result");
				int length = lossValueArray.length();

				for (int i = 0; i < length; i++) {
					JSONObject lossValueObject = (JSONObject) lossValueArray.get(i);

					int[] blues = new int[16];
					int[] reds = new int[33];
					String batchCode = null;

					JSONObject valueObject = lossValueObject.getJSONObject("value");
	...
	private long caculateBetNums(int redSize, int blueSize) {
		long betNums = PublicMethod.zuhe(6, redSize) * PublicMethod.zuhe(1, blueSize);
		return betNums;
	}
	...
	private void addViewAndTouZhu(int redSize, int blueSize) {
		...
		if (betNums > 10000) {
			...
			codeInfo.setTouZhuCode(SsqZiZhiXuanCode.simulateZhuma(selectedRedBallList, selectedBlueBallList));
	...
	private CodeInfo setCodeInfoColor(CodeInfo codeInfo, List<Integer> redList, List<Integer> blueList) {
	...
	public void dialogExcessive(int maxNums) {
		AlertDialog.Builder builder = new AlertDialog.Builder(SimulateSelectNumberActivity.this);
		builder.setTitle(SimulateSelectNumberActivity.this.getString(R.string.toast_touzhu_title).toString());
