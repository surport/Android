...
public class BuyActivityGroup extends ActivityGroup {
	...
	private void createDialog() {
		...
		View popupView = (LinearLayout) inflate.inflate(R.layout.buy_group_window, null);
		...
		final LinearLayout layoutGame = (LinearLayout) popupView.findViewById(R.id.buy_group_layout1);
		final LinearLayout layoutHosity = (LinearLayout) popupView.findViewById(R.id.buy_group_layout2);
		final LinearLayout layoutLuck = (LinearLayout) popupView.findViewById(R.id.buy_group_layout3);
		final LinearLayout layoutQuery = (LinearLayout) popupView.findViewById(R.id.buy_group_layout4);
		final LinearLayout layoutParentLuck = (LinearLayout) popupView.findViewById(R.id.buy_group_one_layout3);
		final LinearLayout layoutPicture = (LinearLayout) popupView.findViewById(R.id.buy_group_layout6);
		final LinearLayout layoutParentPicture = (LinearLayout) popupView.findViewById(R.id.buy_group_one_layout6);
		...
		layoutQuery.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (userno == null || userno.equals("")) {
					...
					Intent intent = new Intent(BuyActivityGroup.this, BetQueryActivity.class);
		...
		if (getIsLuck()) {
			...
			layoutLuck.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// LuckChoose.class);
					Intent intent = new Intent(BuyActivityGroup.this, LuckChoose2.class);
					intent.putExtra("lotno", lotNo);

					// 时时彩加上玩法Id
					if (lotNo == Constants.LOTNO_SSC) {
						intent.putExtra("caipiaoWanfaIndex", mTabHost.getCurrentTab());
					}
		...
		// 获取期号和截止时间
		 Thread thread=new Thread(new Runnable() {		
			...
			public void run() {
				...
				final ForegroundColorSpan  span_RED=new ForegroundColorSpan(Color.RED);  
				final SpannableStringBuilder builder = new SpannableStringBuilder();
				if(!issueStr.equalsIgnoreCase("")){
				try {
				JSONObject allIssue = new JSONObject(issueStr);				
						final String issueStr2 = allIssue.getString("batchcode");
						...
						while(isRun){
							if(isEnd(lesstime)){
								hanler.post(new Runnable(){
									public void run() {
									builder.clear();
								    String lasttime="离截止还剩:"+formatLongToTimeStr(lesstime);
								    builder.append(lasttime);
//								    Log.e("time", lesstime+"");
									builder.setSpan(span_RED, 6, lasttime.length(), Spanned.SPAN_COMPOSING);
...
//										nextIssue();
	...
	public void setlastbatchcode(final String type){
		...
		public void run() {
			// TODO Auto-generated method stub
			final JSONObject prizemore = PrizeInfoInterface.getInstance().getNoticePrizeInfo(type, "1", "1");
			try {
				...
				if(code.equals("0000")){
					...
					tHandler.post(new Runnable() {
						@Override
						public void run() {
							Toast.makeText(BuyActivityGroup.this, msg, Toast.LENGTH_SHORT).show();
						}
					 });
				}
			}catch (JSONException e) {
	...
	public SpannableStringBuilder parseStrforcode(String type,String str){
		...
		if(!str.equals("")){
			...
			if(type.equals(Constants.LOTNO_SSQ)){
				...
				strb.insert(17, ",");
				builder.append(strb);
				ForegroundColorSpan  span_RED=new ForegroundColorSpan(Color.RED);  
				ForegroundColorSpan  span_BULE=new ForegroundColorSpan(Color.BLUE);  
				builder.setSpan(span_RED, 0, strb.length()-2, Spanned.SPAN_COMPOSING);
				builder.setSpan(span_BULE, strb.length()-2, strb.length(), Spanned.SPAN_COMPOSING);
			}
			if(type.equals(Constants.LOTNO_DLT)){
				...
				ForegroundColorSpan  span_RED=new ForegroundColorSpan(Color.RED);  
				ForegroundColorSpan  span_BULE=new ForegroundColorSpan(Color.BLUE);  
				builder.setSpan(span_RED, 0, strb.length()-6, Spanned.SPAN_COMPOSING);
				builder.setSpan(span_BULE, strb.length()-5, strb.length(), Spanned.SPAN_COMPOSING);
			}
			if(type.equals(Constants.LOTNO_FC3D)){
				...
				builder.append(strb);
				ForegroundColorSpan  span_RED=new ForegroundColorSpan(Color.RED);  
				builder.setSpan(span_RED, 0, strb.length(), Spanned.SPAN_COMPOSING);
			}
