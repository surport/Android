...
public class GiftQueryActivity extends Activity {
	...
	final  String GIFTEDUSERNO="toMobileId",GIFTUSERNO = "giftMobileId",USERNO = "userno",STATE = "reciveState",ID = "orderId";
		  ...
		  try {
			  ...
			  for(int i=0;i<winprizejson.length();i++){
				  try{
					...
					winPrizeQueryinfo.setPrizeState(winprizejson.getJSONObject(i).getString("prizeState"));
					winPrizeQueryinfo.setWinCode(winprizejson.getJSONObject(i).getString("winCode"));
					winPrizeQueryinfo.setBetCodeHtml(winprizejson.getJSONObject(i).getString("betCodeHtml"));
					if(type == 0){						
						winPrizeQueryinfo.setToMobileId(winprizejson.getJSONObject(i).getString("toMobileId"));
						winPrizeQueryinfo.setUserNo(winprizejson.getJSONObject(i).getString(GIFTEDUSERNO));
					}else{
						winPrizeQueryinfo.setGiftMobileId(winprizejson.getJSONObject(i).getString("giftMobileId"));
