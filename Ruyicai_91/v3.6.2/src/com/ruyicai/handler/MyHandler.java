	...
	public void handleMessage(Message msg) {
		...
		case RUYICAIHANDLER:
			if (error_code.equals("000000")) {
				...
				if (context instanceof ZiXuanTouZhu || context instanceof com.ruyicai.activity.buy.zixuan.ZiXuanTouZhu
						|| context instanceof JcMainActivity) {
					DirectPayDialog directPayDialog = new DirectPayDialog((Activity) context, context.getResources()
							.getString(R.string.directpay_dialog_title), context.getResources().getString(
							R.string.directpay_dialog_content));
					directPayDialog.showDialog();
					directPayDialog.createDirectPayDialog();
				} else {
					Accoutdialog.getInstance().createAccoutdialog(context,
							context.getResources().getString(R.string.goucai_Account_dialog_msg).toString());
				}
			} else {
				Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
			}
