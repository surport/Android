...
//		if(actions.get(index).infoJson == null){//联网
			...
			contentNet(actions.get(index).getId(),userno);	
	 		}
//		}else{//不联网
//			initContentView();
//		}
	 ...
	 * 联网方法
	 */
	public void contentNet(final String id,final String uerno){
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				String jsonStr = ActionContentInterface.contentQuery(id,uerno);
				progressDialog.dismiss();
				try {
	...
	public void initContentView(){	
		if(actions.get(index).infoJson.getUrl()==null||actions.get(index).infoJson.getUrl().equals("")){
			...
			contentText.setText(actions.get(index).infoJson.getContent());		
				...
				hansurl=true;
				Intent intent=new Intent(ActionActivity.this, ActionDialog.class);
				intent.putExtra("actionurl", actions.get(index).infoJson.getUrl());
