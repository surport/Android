	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		phoneBtn.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// Toast.makeText(getBaseContext(), "暂不支持此功能",
				// Toast.LENGTH_LONG)
				// .show();
			}
				...
				if (phoneET.length() < 11 && phoneVector.isEmpty() == true) {
					...
					if (phoneET.length() < 11 && phoneET.length() > 0) {
						...
						if (sessionid.equals("")) {
							...
							startActivity(intentSession);
							return;
						} else if (jrtLot.ifPerfectIfo(
								RuyiExpressFeelSuccess.this, sessionid)) {// 身份验证
							if (confirm) {
								// 需要判断后弹出提示框
								// 周黎鸣 7.5 代码修改：添加登陆判断
								...
								Thread t = new Thread(new Runnable() {
									...
									public void run() {
										...
										// }
										String aToPhoneNum = "";
										// while (iRetrytimes < 3 && iRetrytimes
										// > 0) {
										//						
										// try {
										// 获取接受手机号
										...
										if ("ssqJixuan".equals(successStr)
											...
											if (!phoneET.getText().toString()
													.equals("")
													&& phoneVector.isEmpty() == false) {// phoneVector.isEmpty()==true&&
												// if(phoneVector.isEmpty()==false){
												toPhoneNum += phoneET.getText()
														.toString()
														+ ",";
										...
										if ("ssqJixuan".equals(successStr)) {
											...
											betcode += GT.betCodeToStringTC(3,
											//														
													iTempRand);
											...
											System.out
													.println("---------iTempRandString----------"
															+ iTempRandString);
											iNumbers.add(iTempRandString);
											betcode += GT.betCodeToStringTC(4,
													iTempRand);
										...
										if ("ssqJixuan".equals(successStr)
											...
											while (iRetrytimes < 3
												...
												try {
													...
													String codeResponse = codeResponse1
															.substring(0, index);
													// 获取返回码的前6个字符
											...
											PublicMethod.myOutput("??????????"
													+ error_code);
											if (error_code
													.equalsIgnoreCase("000000")) {// equals("000000")
												// -----------怎样回到主列表
												try {
													batchCode = obj
