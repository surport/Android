	...
	public void update() {
		try {
			...
			try {
				...
				amt = (Integer) obj.get("caseBuyAfterAmt") * 100;
			...
			if (lest == 0 || amt == 100) {
				...
				findViewById(R.id.join_rengou).setVisibility(View.GONE);
			}
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		buttonBuy.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (sessionid.equals("")) {
					...
					Toast
							.makeText(JoinIn.this, "认购份数不能为空！",
									Toast.LENGTH_SHORT).show();
				} else if (Integer.parseInt(num) == 0) {
					Toast.makeText(JoinIn.this, "认购份数至少为一份！",
							Toast.LENGTH_SHORT).show();
					editNum.setText("");
