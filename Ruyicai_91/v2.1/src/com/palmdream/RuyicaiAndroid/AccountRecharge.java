...
public class AccountRecharge extends Activity implements MyDialogListener {
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 8:
				// 取消联网对话框 2010/7/9 陈晨
				progressDialog.dismiss();
				Toast.makeText(getBaseContext(), "网络异常，请检查网络连接！",
						Toast.LENGTH_LONG).show();
			...
			case 9:
				// 取消联网对话框 2010/7/9 陈晨
				progressDialog.dismiss();
				Toast.makeText(getBaseContext(), "充值失败，请确认无误后再充值！",
						Toast.LENGTH_LONG).show();
				break;
			// 新添的处理消息 2010/7/9陈晨
			case 10:
			...
			case 15:
				progressDialog.dismiss();
				Toast.makeText(AccountRecharge.this, "系统正在结算...",
						Toast.LENGTH_SHORT).show();
				break;
			case 16:
				progressDialog.dismiss();
			...
			case 17:
				progressDialog.dismiss();
				Toast.makeText(getBaseContext(), "查询失败，请稍后...",
						Toast.LENGTH_LONG).show();
				break;
			case 18:
				progressDialog.dismiss();
			...
			case 21:
				...
				Intent intent2 = new Intent(AccountRecharge.this,
						UserLogin.class);
	...
	protected Dialog onCreateDialog(int i) {
		...
		case BANK_CARD_PHONE_NO_DIALOG:// 银行卡电话卡充值(未绑定)
			final View bank_card_phone_view = factory.inflate(
					R.layout.bank_card_phone_dialog, null);
			return new AlertDialog.Builder(this).setIcon(
					R.drawable.bank_cards_phone_recharge1).setTitle(
		...
		if ((!(num.equals("")) && num != null)
			...
			if (num.length() == 11) {
				if (Integer.parseInt(value) >= 20) {
					...
					bank_card_id = bankid;
					bank_phone_card_net(phonenum, value, bank_card_id,
							sessionid);
					bank_card_phone_recharge_value.setText("");
					bank_card_phone_phone_num.setText("");
					bank_card_phone_bankid.setText("");
	...
	public void dialogDNA(String str) {
		...
		try {
			...
			// certAddress = obj.getString("CertAddress");
			// bindDate = obj.getString("BindDate");
