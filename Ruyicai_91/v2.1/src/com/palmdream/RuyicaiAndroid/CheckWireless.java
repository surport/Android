	...
	private void checkWirelessStatus() {
		...
		if (activeNetInfo != null) {
			...
			if (connectivityManager.getNetworkInfo(1).getState() == NetworkInfo.State.CONNECTED) {
				connectWIFI = true;
				// Toast.makeText( context, "WIFI Network Type : " +
				// connectivityManager.getNetworkInfo(1).getTypeName(),
				// Toast.LENGTH_LONG ).show();

			}
			if (connectivityManager.getNetworkInfo(0).getState() == NetworkInfo.State.CONNECTED)
				connectGPRS = true;
			// Toast.makeText( context, "GPRS Network Type : " +
			// connectivityManager.getNetworkInfo(0).getTypeName(),
			// Toast.LENGTH_LONG ).show();
