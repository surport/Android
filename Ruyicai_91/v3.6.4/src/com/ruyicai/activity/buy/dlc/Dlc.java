			...
			try {
				...
				if(code.equals("0000")){
					...
					tHandler.post(new Runnable() {
						@Override
						public void run() {
							Toast.makeText(Dlc.this, msg, Toast.LENGTH_SHORT).show();
						}
					 });
				}
		...
		if(!str.equals("")){
			...
			if(type.equals(Constants.LOTNO_ten)){
				...
				for (int i = 0; i < strgd.length; i++) {
					...
					builder.setSpan(new ForegroundColorSpan(Color.RED), upLength, code.length() + upLength,
								Spanned.SPAN_COMPOSING);
					}else{
						builder.setSpan(new ForegroundColorSpan(Color.BLUE), upLength, code.length() + upLength,
								Spanned.SPAN_COMPOSING);	
					}
					if(i!=strgd.length-1){
					builder.append(",");
	 ...
	 /**
	   ...
	   public int getZhuShu(){
		   ...
		   if(isJiXuan){
			...
			if(state.equals("Q2")){
		    	   zhushu = getzhushuQ2(areaNums[0].table.getHighlightStr(), areaNums[1].table.getHighlightStr())*iProgressBeishu;
			}else if(state.equals("Q3")){				   
		    	   zhushu = getzhushuQ3(areaNums[0].table.getHighlightStr(), areaNums[1].table.getHighlightStr(), areaNums[2].table.getHighlightStr())*iProgressBeishu;
			}else{
				   int ballNums = areaNums[0].table.getHighlightBallNums();
