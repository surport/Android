...
public class JoinDetailActivity extends Activity implements HandlerMsg {
	private TextView name, describe, atm, id, renAtm, baoAtm, progress, state, shengAtm, person, deduct, content,
			amountProgress, amountText, safeProgress, safeText, minText, minText1, lotnotext, beishutext,
			batchcodetext, faqirengou, timeText, rengouText, minRGText;
	private LinearLayout starLayout;
	private LinearLayout faqixinxi, fanganxiangqing, fanganleirong, rengoushezhi, fenxianglayout;
	private Button faqi, xiangqing, leirong, rengou, canyu;
	private boolean isfaqi = false, isxiangqing = false, isleirong = false, isrengou = false, iscanyu = true;
	...
	public void init() {
		...
		dingBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent intent = new Intent(JoinDetailActivity.this, JoinDingActivity.class);
		...
		leavTextView(safeText, false);
		// linshi
		PublicMethod.createStar(starLayout, detatil.getCrown(), detatil.getGrayCrown(), detatil.getCup(),
				detatil.getGrayCup(), detatil.getDiamond(), detatil.getGrayDiamond(), detatil.getStar(),
				detatil.getGrayStarNum(), JoinDetailActivity.this, 0);
	...
	public void tenoauth() {
		...
		tencent_access_token_secret = shellRW.getStringValue("tencent_access_token_secret");
		if (tencent_token.equals("") && tencent_access_token_secret.equals("")) {
			...
			Intent intent = new Intent(JoinDetailActivity.this, OAuthV1AuthorizeWebView.class);// 创建Intent，使用WebView让用户授权
			...
			Intent intent = new Intent(JoinDetailActivity.this, TencentShareActivity.class);
	...
	public JoinDetatil getValue() {
		...
		try {
			...
			detatil.setLotnoJc(json.getString("lotNo"));
			detatil.setJson(new JSONObject(json.getString("contentJson")));
			if (json.has("endTime")) {
				detatil.setTime(json.getString("endTime"));
			}
			Constants.shareContent = "Hi，我刚使用了如意彩手机客户端买彩票，很方便呢！" + "你也试试吧，彩票随身投，大奖时时有！中奖了记的要请客啊！地址:" + detatil.getUrl();
			Constants.source = "4";
			...
			try {
				detatil.setGrayCup(displayIcon.getString("graycup"));
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			try {
			...
			try {
				detatil.setGrayDiamond(displayIcon.getString("graydiamond"));
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			try {
			...
			try {
				detatil.setGrayStarNum(displayIcon.getString("graygoldStar"));
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			try {
			...
			try {
				detatil.setGrayCrown(displayIcon.getString("graycrown"));
			} catch (Exception e) {
				e.printStackTrace();
			}
		} catch (Exception e) {
	...
	public void setValue(JoinDetatil detatil) {
		...
		if(lotno.equals(Constants.LOTNO_JCLQ)||lotno.equals(Constants.LOTNO_JCZQ)){
			timeText.setVisibility(View.GONE);
		}else{
			timeText.append(detatil.getTime());
		}
...
//		content.append(detatil.getContent());
		contentListView.createListContent(layoutMain,content,detatil.getLotnoJc(),detatil.getContent(),detatil.getJson());
		amountEdit.setText(detatil.getMinAmt());
		String minStr = "";
		int minInt = Integer.parseInt(detatil.getMinAmt());
		int rengouInt = Integer.parseInt(detatil.getShengAtm());
		if (rengouInt < minInt) {
		...
		PublicMethod.setTextColor(minRGText, 5, minStr.length() - 2, minStr,Color.RED);
		PublicMethod.setTextColor(rengouText, 6, rengouStr.length() - 1, rengouStr,Color.RED);
		amountProgress.setText("占总额" + progress(amountEdit.getText().toString(), detatil.getAtm()) + "%");
		safeProgress.setText("占总额" + progress(safeAmtEdit.getText().toString(), detatil.getAtm()) + "%");
		...
		leavTextView(safeText, false);
		PublicMethod.createStar(starLayout, detatil.getCrown(), detatil.getGrayCrown(), detatil.getCup(),
				detatil.getGrayCup(), detatil.getDiamond(), detatil.getGrayDiamond(), detatil.getStarNum(),
				detatil.getGrayStarNum(), JoinDetailActivity.this, 0);
	...
	public void isJoinInNet() {
		...
		if (amount.equals("") && safeAmt.equals("")) {
			...
			Toast.makeText(JoinDetailActivity.this, "请您至少认购" + minInt + "元", Toast.LENGTH_SHORT).show();
		}
	...
	public void joinInNet() {
		...
		Thread t = new Thread(new Runnable() {
			...
			@Override
			public void run() {
				str = JoinInInterface.betLotJoin(userno, phonenum, caseId, PublicMethod.toFen(isNull(amount)),
	...
	public void joinCanyuNet() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				String str = "00";
				str = QueryJoinCanyuInterface.queryLotJoincanyu(caseId, userno, phonenum, String.valueOf(pageindex),
						"10");
				try {
					json = new JSONObject(str);
					String msg = json.getString("message");
	...
	private void getMore() {
		...
		if (pageindex < allpage) {
			...
			pageindex = allpage - 1;
			Toast.makeText(JoinDetailActivity.this, "已至尾页", Toast.LENGTH_SHORT).show();
		}
	}
	...
	public void joinCannelIdNet() {
		...
		Thread t = new Thread(new Runnable() {
			...
			@Override
			public void run() {
				str = JoinCannelIdInterface.joincanelid(caseId, userno, phonenum);
				try {
					json = new JSONObject(str);
					final String msg = json.getString("message");
					String error = json.getString("error_code");
	...
	public void succeedDialog(String title, String string) {
		...
		Dialog dialog = new AlertDialog.Builder(this).setMessage(string).setTitle(title)
	 ...
	 */
	class JoinDetatil {
		private String name = "", star = "", describe = "", atm = "0", id = "", renAtm = "0", baoAtm = "0",
				progress = "", state = "", shengAtm = "0", person = "", deduct = "", content = "";
		...
		public String getLotnoJc() {
			return lotnoJc;
		}

		public void setLotnoJc(String lotno) {
			this.lotnoJc = lotno;
		}
