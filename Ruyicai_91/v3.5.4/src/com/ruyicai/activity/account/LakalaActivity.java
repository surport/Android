...
public class LakalaActivity extends Activity {
	...
	private static JSONObject getJSONByLotno(){
		JSONObject  jsonObjectByLotno =  RechargeDescribeInterface.getInstance().rechargeDescribe("lakalaChargeDescription");
	...
	private void isTransaction(Intent intent) {
		...
		if(null!=isTransactionOk&&isTransactionOk.equals("true")){
			Toast.makeText(this, R.string.transactionOK, Toast.LENGTH_LONG).show();
		}else if(null!=isTransactionOk&&isTransactionOk.equals("false")){
			Toast.makeText(this, R.string.transactionErr, Toast.LENGTH_LONG).show();
		}
	}
	@Override
    ...
    private void beginYinpayRecharge(View Vi) {
		...
		RWSharedPreferences pre = new RWSharedPreferences(LakalaActivity.this, "addInfo");
		String sessionId = pre.getStringValue(ShellRWConstants.SESSIONID);
		String userno = pre.getStringValue(ShellRWConstants.USERNO);
		if (sessionId.equals("")) {
			Intent intentSession = new Intent(LakalaActivity.this,UserLogin.class);
...
//	    	String merPswd = obj.getString("");
			...
			String randNum = obj.getString("randNum");
	    	turnYinView(ver,merId,minCode,order,amount,time,macType,desc,randnum,productName,randNum,mac,url);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
     ...
     * 拉卡拉充值跳转到插件
     */
    public void turnYinView(String ver,
			String merId,String minCode,String order,String amount,
			String time,String macType,
			String desc,String randnum,String productName,String randNum,String mac,String url){
    	try{
    		 ...
    		 * 测试
    		 */
    		addShortcutDateForIntent(intent,ver, merId, minCode, order,amount,time,macType,desc,randnum,productName,randNum,mac);
	...
	private void addShortcutDateForIntent(Intent intent,String ver,
			String merId,String minCode,String order,String amount,
			String time,String macType,
			String desc,String randnum,String productName,String randNum,String mac) {
...
//		bundle.putString("merClassName", getComponentName().getClassName());//交易成功，返回商户应用程序类名
	...
	private void createDialogDownloadApp(String softwareurl) {
		HomeUpdate update = new HomeUpdate(this,new Handler(), softwareurl, getString(R.string.isInstall),getString(R.string.toast_install_lakala));
