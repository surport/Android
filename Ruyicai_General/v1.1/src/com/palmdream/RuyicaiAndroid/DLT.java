...
public class DLT extends Activity // 事件 回调函数
	...
	int topButtonStringId[] = { R.string.danshi, R.string.fushi,
			R.string.dantuo, R.string.twoin12 };
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 4:
				progressdialog.dismiss();
				Toast.makeText(getBaseContext(), "投注成功，出票成功", Toast.LENGTH_LONG)
						.show();
				if(iCurrentButton== PublicConst.BUY_DLT_DANSHI||iCurrentButton== PublicConst.BUY_DLT_FUSHI){
...
//				投注成功后清除小球  陈晨  20100728
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (publicTopButton == PublicConst.BUY_DLT_DANSHI) {
			create_DLT_DANSHI();
			changeTextSumMoney();
		} else if (publicTopButton == PublicConst.BUY_DLT_FUSHI) {
			create_DLT_FUSHI();
			changeTextSumMoney();
	...
	private void createBuyView(int aWhichBuy) {
		...
		switch (aWhichBuy) {
		case PublicConst.BUY_DLT_DANSHI: // 单式
			create_DLT_DANSHI();
			break;
	...
	private void create_DLT_TWOIN12(){
				...
				R.layout.layout_dlt_twoin12, null);
		{
			...
			setSeekWhenAddOrSub(R.id.dlt_twoin12_seekbar_add_beishu, iV, 1,
					mSeekBarBeishu, true);
			setSeekWhenAddOrSub(R.id.dlt_twoin12_seekbar_subtract_qihao, iV, -1,
					mSeekBarQishu, false);
		...
		mTimesMoney = 2;
//		mZhuijia = (CheckBox) this.findViewById(R.id.dlt_twoin12_zhuijia_touzhu_cb);
//		mZhuijia.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
//
//			@Override
...
//												mBallHolder, 0);
	...
	private void create_DLT_DANTUO() {
				...
				R.layout.layout_dlt_dantuo, null);
		{
			...
			redBallTable = makeBallTable(iV, R.id.table_front_danma,iScreenWidth, redBallViewNum, redBallViewWidth,redBallResId, RED_BALL_START);
			redTuoBallTable = makeBallTable(iV, R.id.table_front_tuoma,	iScreenWidth, redBallViewNum, redBallViewWidth,redBallResId, RED_TUO_BALL_START);

			int blueBallViewNum = 12;
			blueBallTable = makeBallTable(iV, R.id.table_rear_danma,iScreenWidth, blueBallViewNum, redBallViewWidth,blueBallResId, BLUE_BALL_START);

			blueTuoBallTable = makeBallTable(iV, R.id.table_rear_tuoma,iScreenWidth, blueBallViewNum, redBallViewWidth,blueTuoBallResId, BLUE_TUO_BALL_START);
		...
		buyView.addView(iV, new LinearLayout.LayoutParams(buyView
				.getLayoutParams().width, buyView.getLayoutParams().height));
		mZhuijia = (CheckBox) this.findViewById(R.id.dlt_dantuo_zhuijia_touzhu_cb);
		mZhuijia.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
	...
	private void create_DLT_FUSHI() {
		...
		buyView.addView(iV, new LinearLayout.LayoutParams(buyView
				.getLayoutParams().width, buyView.getLayoutParams().height));
		mZhuijia = (CheckBox) this.findViewById(R.id.dlt_fushi_zhuijia_touzhu_cb);
		mZhuijia.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
	...
	/*
	 * 创建单式购彩View @return void
	 */
	private void create_DLT_DANSHI() {
		buyView.removeAllViews();
		// View iV = View.inflate(this, R.layout.layout_dlt_danshi, null);
		LayoutInflater inflate = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		LinearLayout iV = (LinearLayout) inflate.inflate(
				R.layout.layout_dlt_danshi, null);
		{
			int redBallViewNum = 35;
			int redBallViewWidth = DLT.BALL_WIDTH;
			iScreenWidth = PublicMethod.getDisplayWidth(this);
			...
			mTextSumMoney = (TextView) iV
					.findViewById(R.id.dlt_danshi_text_sum_money);
			mTextSumMoney.setText(getResources().getString(
					R.string.please_choose_number));
			mSeekBarBeishu = (SeekBar) iV
					.findViewById(R.id.dlt_danshi_seek_beishu);
			mSeekBarBeishu.setOnSeekBarChangeListener(this);
			mSeekBarBeishu.setProgress(iProgressBeishu);

			mSeekBarQishu = (SeekBar) iV
					.findViewById(R.id.dlt_danshi_seek_qishu);
			mSeekBarQishu.setOnSeekBarChangeListener(this);
			mSeekBarQishu.setProgress(iProgressQishu);

			mTextBeishu = (TextView) iV
					.findViewById(R.id.dlt_danshi_text_beishu);
			mTextBeishu.setText("" + iProgressBeishu);
			mTextQishu = (TextView) iV.findViewById(R.id.dlt_danshi_text_qishu);
			mTextQishu.setText("" + iProgressQishu);
			/*
			 * 点击加减图标，对seekbar进行设置： @param int idFind, 图标的id View iV, 当前的view
			 * final int isAdd, 1表示加 -1表示减 final SeekBar mSeekBar @return void
			 */
			setSeekWhenAddOrSub(R.id.dlt_danshi_seekbar_subtract_beishu, iV,
					-1, mSeekBarBeishu, true);
			setSeekWhenAddOrSub(R.id.dlt_danshi_seekbar_add_beishu, iV, 1,
					mSeekBarBeishu, true);
			setSeekWhenAddOrSub(R.id.dlt_danshi_seekbar_subtract_qihao, iV, -1,
					mSeekBarQishu, false);
			setSeekWhenAddOrSub(R.id.dlt_danshi_seekbar_add_qihao, iV, 1,
					mSeekBarQishu, false);

			dlt_btn_newSelect = (Button) iV
					.findViewById(R.id.dlt_danshi_btn_newSelect);
			dlt_btn_newSelect.setOnClickListener(new Button.OnClickListener() {

				@Override
				public void onClick(View v) {
					mBallHolder = ballBetPublicClass.new BallHolder();
					mBallHolder = redBallTable.randomChooseConfigChange(5,
							mBallHolder, 0);
					mBallHolder = blueBallTable.randomChooseConfigChange(2,
							mBallHolder, 1);
					changeTextSumMoney();
				}

			});
			dlt_b_touzhu_danshi = (ImageButton) iV
					.findViewById(R.id.dlt_danshi_b_touzhu);
			dlt_b_touzhu_danshi.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					beginTouZhu(); // 1表示当前为单式
				}
		...
		buyView.addView(iV, new LinearLayout.LayoutParams(buyView
				.getLayoutParams().width, buyView.getLayoutParams().height));
		mZhuijia = (CheckBox) this.findViewById(R.id.dlt_danshi_zhuijia_touzhu_cb);
		mZhuijia.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
		...
		mCheckBox = (CheckBox) this.findViewById(R.id.dlt_danshi_jixuan_cb);
		if (mBallHolder.flag == 1) {
			mCheckBox.setChecked(mBallHolder.DanShi.bCheckBox);
			if (mBallHolder.DanShi.bCheckBox)
				dlt_btn_newSelect.setVisibility(View.VISIBLE);
				...
				.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					...
					public void onCheckedChanged(CompoundButton buttonView,
						...
						if (isChecked) {
							if (mBallHolder.flag != 1) {
								mBallHolder = ballBetPublicClass.new BallHolder();
								// redBallTable.randomChoose(5);
								// blueBallTable.randomChoose(2);
								mBallHolder = redBallTable
										.randomChooseConfigChange(5,
												mBallHolder, 0);

								mBallHolder = blueBallTable
										.randomChooseConfigChange(2,
												mBallHolder, 1);
	...
	public void onProgressChanged(SeekBar seekBar, int progress,
		...
		// 单式
		case R.id.dlt_danshi_seek_beishu:
		...
		case R.id.dlt_twoin12_seek_beishu:
			...
			changeTextSumMoney();
			break;
		case R.id.dlt_danshi_seek_qishu:
...
//	}
	 ...
	 * @param int aBlueBalls 篮球个数 @return long 注数
	 */
	private long getDLTDTZhuShu(int aRedBalls, int aRedTuoBalls, int aBlueBalls,int aBlueTuoBalls) {// 得到大乐透胆拖的注数
		long dltZhuShu = 0L;
		iSendZhushu = 0L;
	...
	/*
	 * 复式玩法注数计算方法 @param int aRedBalls 红球个数 @param int aBlueBalls 篮球个数 @return
	 * long 注数
	 */
	...
	public void changeTextSumMoney() {
		...
		case PublicConst.BUY_DLT_DANSHI:
			...
			if (redBallTable.getHighlightBallNums() < 5) {
				...
				if (blueBallTable.getHighlightBallNums() < 2) {
					...
					String iTempString = "共" + iZhuShu + "注，共" + (iZhuShu * mTimesMoney)
							+ "元";
		...
		case PublicConst.BUY_DLT_DANTUO: {
			...
			if (iRedHighlights + iRedTuoHighlights < 5) {
				mTextSumMoney.setText(getResources().getString(
	...
	private void buy_DANTUO(int aWhichGroupBall, int aBallViewId) {
		...
		if (aWhichGroupBall == 0) { // 红球 胆区 最多4个小球
			...
			// mBallHolder.flag=1;
		} else if (aWhichGroupBall == 1) { // 篮球区胆区
			...
			if (isHighLight == PublicConst.BALL_TO_HIGHLIGHT) {
				...
				mBallHolder.DanShi.iTuoRedBallStatus[aBallViewId] = 0;
			// mBallHolder.flag=1;
		}else if (aWhichGroupBall == 3) { // 篮球 拖区 最多5个小球
			int iChosenBallSum = 12;
	...
	public void onOkClick(int[] aNums) {
		...
		if (aNums.length == 2) {
			...
			for (j = 0; j < iDantuoBlueNumber; j++) {
				// redBallTable.changeBallState(1, iTotalRandoms[i]);
...
//			mBallHolder = blueTuoBallTable.randomChooseConfigChange(
//					iDantuoBlueTuoNumber, mBallHolder, 1);
			changeTextSumMoney();
		}else if (aNums.length == 1) {
			iTwoin12BlueBallNumber = aNums[0];
			PublicMethod.myOutput("*********** iTwoin12BlueBallNumber " + iTwoin12BlueBallNumber);
			mBallHolder = blueBallTable.randomChooseConfigChange(
					iTwoin12BlueBallNumber, mBallHolder, 1);
			changeTextSumMoney();
		}
		/*
		 * switch(iCurrentButton){ case PublicConst.BUY_DLT_DANSHI: break; case
		 * PublicConst.BUY_DLT_FUSHI: iFushiRedBallNumber = aRedNum;
		 ...
		 * case PublicConst.BUY_DLT_DANTUO: break; default: break; }
		 */
	}
	...
	private void beginTouZhu() {
		...
		if (iCurrentButton == PublicConst.BUY_DLT_DANSHI) { // 1为单式投注
//			PublicMethod.myOutput("*************touzhu   " + zhuma_danshi());
			if (sessionIdStr.equals("")) {
				Intent intentSession = new Intent(DLT.this, UserLogin.class);
				startActivity(intentSession);
			} else if (redBallTable.getHighlightBallNums() != 5
					&& blueBallTable.getHighlightBallNums() != 2) {
				alert1("请选择5个前区号码和2个后区号码");
			} else if (redBallTable.getHighlightBallNums() != 5) {
				alert1("请选择5个前区号码");
		...
		if (iCurrentButton == PublicConst.BUY_DLT_FUSHI) {
			...
			// ShellRWSharesPreferences(dlttest.this , "addInfo");
			// String sessionIdStr = pre.getUserLoginInfo("sessionid");
			if (sessionIdStr.equals("")) {
				Intent intentSession = new Intent(DLT.this, UserLogin.class);
				startActivity(intentSession);
				...
				// alert("您选择的是"+iZhuShu+"注大乐透复式彩票，"+"共计"+(iZhuShu*2)+"元");
		...
		if (iCurrentButton == PublicConst.BUY_DLT_DANTUO) {
			...
			if (sessionIdStr.equals("")) {
				Intent intentSession = new Intent(DLT.this, UserLogin.class);
				startActivity(intentSession);
			} else if (redNumber + redTuoNumber < 5
					|| redNumber + redTuoNumber > 22 || redTuoNumber < 1
					|| blueTuoNumber < 1 || redNumber > 4
					|| blueTuoNumber+blueNumber<2) {
				alert1("请选择:\n 0~4个前区胆码；\n" + " 1~22个前区拖码；\n" + " 0~1个后区胆码；\n"
						+ " 1~12个后区拖码；\n" + " 前区号码5~22个；\n" + " 后区号码2~12个；\n");
			}else if (iZhuShu > 10000) {
				DialogExcessive();
				...
				// int iZhuShu=getZhuShu();
				// //String iTempString="共"+iZhuShu+"注，共"+(iZhuShu*2)+"元";
				// alert("您选择的是"+iZhuShu+"注大乐透胆拖彩票，"+"共计"+(iZhuShu*2)+"元");
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						Thread t = new Thread(new Runnable() {
							...
							public void run() {
								...
								// 20100711 cc 新接口
								if(mTimesMoney==2){
									strCode = payNew(zhuma_dantuo, iBeiShu + "", iZhuShu
										* 2 + "",iQiShu+"");
								}else if(mTimesMoney==3){
									strCode = payNew(zhuma_dantuo, iBeiShu + "", iZhuShu
											* 3 + "",iQiShu+"");
...
//								} 
	...
	protected String[] payNew(String betCode, String lotMulti, String amount,String qiShu) {
		...
		String phonenum = shellRW.getUserLoginInfo("phonenum");
		PublicMethod.myOutput("-------------touzhusessionid" + sessionid);
		PublicMethod.myOutput("-------------phonenum" + phonenum+"----lotMulti---"+lotMulti+"----amount----"+amount+"---qiShu---"+qiShu);
        if(mTimesMoney==2){
		error_code = betting.BettingTC(phonenum , "T01001", betCode, lotMulti, amount,"2",qiShu,sessionid);
        }else if(mTimesMoney==3){
        error_code = betting.BettingTC(phonenum , "T01001", betCode, lotMulti, amount,"3",qiShu,sessionid);
        }
...
//		}
	...
	private void DialogExcessive() {
		...
		builder.setMessage("单张彩票基本投注的最大投注金额不超过20000元，基本投注加追加投注的最大投注金额不超过30000元。");
		// 确定
