...
public class PL3 extends Activity implements OnClickListener,
	...
	int topButtonStringId[] = { R.string.zhixuan, R.string.zuxuan3,
			R.string.zuxuan6, R.string.hezhi };
...
//				清除高亮小球 陈晨 20100728
			...
			case 7:
				...
				Intent intentSession = new Intent(PL3.this,
						UserLogin.class);
					...
					.setOnClickListener(new TextView.OnClickListener() {
						...
						public void onClick(View v) {
							if (mBallHolder.flag != 1) {
								...
								PL3_hezhi_zhixuan.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user));
								PL3_hezhi_zu3.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user_d));
								PL3_hezhi_zu6.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user_d));
							}
			...
			PL3_hezhi_zu3.setOnClickListener(new TextView.OnClickListener() {
				...
				public void onClick(View v) {
					if (mBallHolder.flag != 1) {
						...
						PL3_hezhi_zhixuan.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user_d));
						PL3_hezhi_zu3.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user));
						PL3_hezhi_zu6.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user_d));
					}
				}
			...
			PL3_hezhi_zu6.setOnClickListener(new TextView.OnClickListener() {
				...
				public void onClick(View v) {
					if (mBallHolder.flag != 1) {
						...
						PL3_hezhi_zhixuan.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user_d));
						PL3_hezhi_zu3.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user_d));
						PL3_hezhi_zu6.setBackgroundDrawable(getResources().getDrawable(R.drawable.frame_rectangle_user));
					}
				}
	...
	private void create_PL3_HEZHI_ZU6() {
		...
		if (buyView.findViewById(R.id.pl3_hezhi_zu3_linearlayout) != null) {
			buyView.removeView(buyView.findViewById(R.id.pl3_hezhi_zu3_linearlayout));
		}
		if (buyView.findViewById(R.id.pl3_hezhi_zhixuan_linearlayout) != null) {
			buyView.removeView(buyView.findViewById(R.id.pl3_hezhi_zhixuan_linearlayout));
		}
	...
	private void create_PL3_HEZHI_ZHIXUAN() {
		...
		if (buyView.findViewById(R.id.pl3_hezhi_zhixuan_linearlayout) == null) {
			...
			// 当屏幕切换的时候绘图以后，把已经高亮的小球显示出来
			if (mBallHolder.flag == 1) {
				PL3HezhiZhixuanBallTable.changeBallStateConfigChange(mBallHolder.HezhiZhixuanBallGroup.iHezhiZhixuanBallStatus);
				mSeekBarBeishu.setProgress(mBallHolder.HezhiZhixuanBallGroup.iBeiShu);
				mSeekBarQishu.setProgress(mBallHolder.HezhiZhixuanBallGroup.iQiShu);
				mCheckBox.setChecked(mBallHolder.HezhiZhixuanBallGroup.bCheckBox);
	...
	private void create_PL3_ZU3() {
				...
				null);
		{
			...
			PL3Zu3FushiBallTable = makeBallTable(linearLayout,
					...
					redBallViewWidth, PL3BallResId, RED_PL3_ZU3_FUSHI_START,
					0);
			// 默认是单式，所以复式不可用
	...
	private int getPL3ZhixuanHezhiZhushu() {
		...
		int[] BallNoZhushus = {1,3,6,10,15,21,28,36,45,55,63,69,73,75,75,73,69,63,55,45,36,28,21,15,10,6,3,1};// 0~27
	...
	private int getPL3Zu3HezhiZhushu() {
		...
		int[] BallNoZhushus = {1,2,1,3,3,3,4,5,4,5,5,4,5,5,4,5,5,4,5,4,3,3,3,1,2,1};// 1~26
	...
	private int getPL3Zu6HezhiZhushu() {
		...
		int[] BallNoZhushus = {1,1,2,3,4,5,7,8,9,10,10,10,10,9,8,7,5,4,3,2,1,1};// 3~24
					...
					for (int j = 0; j < BallNoZhushus.length; j++) {
						if (j == BallNos[i] - 1) {// 因为数组是从0开始的，小球号码从1开始，故减去1
							...
							String temp = "当前玩法为和值直选，共"
									+ iZhuShu * iProgressBeishu + "注，共" + (iZhuShu * iProgressBeishu * 2) + "元";
	 ...
	 * 和值组6
	 * 
	 * @param aWhichGroupBall 第几组小球
	 * @param aBallViewId 被click小球的id，从3开始计数，小球上显示的数字为id
	 */
	private void buy_PL3_HEZHI_ZU6(int aWhichGroupBall, int aBallViewId) {
		if (aWhichGroupBall == 12) { // 和值组6
			int iChosenBallSum = 9;
			int isHighLight = PL3HezhiZu6BallTable.changeBallState(iChosenBallSum, aBallViewId);
	 ...
	 * 
	 * @param aWhichGroupBall 第几组小球
	 * @param aBallViewId 被click小球的id，从0开始计数，小球上显示的数字为id
	 */
	private void buy_PL3_HEZHI_ZU3(int aWhichGroupBall, int aBallViewId) {
		if (aWhichGroupBall == 11) { // 和值组3
			int iChosenBallSum = 9;
			int isHighLight = PL3HezhiZu3BallTable.changeBallState(iChosenBallSum, aBallViewId);
			if (isHighLight == PublicConst.BALL_TO_HIGHLIGHT) {
	...
	private void buy_PL3_HEZHI_ZHIXUAN(int aWhichGroupBall, int aBallViewId) {
		if (aWhichGroupBall == 10) { // 和值直选
			int iChosenBallSum = 9;
			int isHighLight = PL3HezhiZhixuanBallTable.changeBallState(iChosenBallSum, aBallViewId);
			if (isHighLight == PublicConst.BALL_TO_HIGHLIGHT) {
	...
	private void buy_PL3_ZHIXUAN(int aWhichGroupBall, int aBallViewId) {
		if (aWhichGroupBall == 7) { // 百位
			...
			// 每次点击后记住小球的状态
			int isHighLight = PL3ZhixuanBaiweiBallTable.changeBallState(iChosenBallSum, aBallViewId);
			if (isHighLight == PublicConst.BALL_TO_HIGHLIGHT) {
			...
			if (iCurrentButton == PublicConst.BUY_PL3_ZU3) {
				...
				if (bZu3Danshi) {
					...
					if (baiweiNums < 1 || shiweiNums < 1 || geweiNums < 1) {
						...
						String baiweistr = PL3A1Zu3DanshiBallTable.getHighlightBallNOs()[0]- 1 + "";// 前2位相同
						String geweistr = PL3BZu3DanshiBallTable.getHighlightBallNOs()[0]- 1 + "";// 前2位相同
						if (iZhuShu * 2 > 100000) {
							alertDialog("投注失败","单笔投注不能大于100000元");
						} else {
				...
				if (bZu3Fushi) {
					if (PL3Zu3FushiBallTable.getHighlightBallNums() < 2) {
						...
						int[] fushiNums = PL3Zu3FushiBallTable.getHighlightBallNOs();
						String fushiStr = "";
			...
			if (iCurrentButton == PublicConst.BUY_PL3_HEZHI) {
				...
				if (iWhich == 10) {
					...
					if (PL3HezhiZhixuanBallTable.getHighlightBallNums() < 1) {
						...
						int iZhuShu = getZhuShu() * iProgressBeishu;
						String fushiStr="";
						int[] zhuma_zhixuanhezhi = PL3HezhiZhixuanBallTable.getHighlightBallNOs();
				...
				if (iWhich == 11) {
					...
					if (PL3HezhiZu3BallTable.getHighlightBallNums() < 1) {
						...
						int iZhuShu = getZhuShu() * iProgressBeishu;
						String fushiStr="";
						int[] zhuma_zu3hezhi = PL3HezhiZu3BallTable.getHighlightBallNOs();
				...
				if (iWhich == 12) {
					...
					if (PL3HezhiZu6BallTable.getHighlightBallNums() < 1) {
						...
						int iZhuShu = getZhuShu() * iProgressBeishu;
						String fushiStr="";
						int[] zhuma_zu6hezhi = PL3HezhiZu6BallTable.getHighlightBallNOs();
	...
	public void onOkClick(int[] aReturn) {
		...
		switch (iCurrentButton) {
		case PublicConst.BUY_PL3_ZHIXUAN:
			mBallHolder = PL3ZhixuanBaiweiBallTable.randomChooseConfigChangePL3(aReturn[0], mBallHolder, 7,null);
			mBallHolder = PL3ZhixuanShiweiBallTable.randomChooseConfigChangePL3(aReturn[1], mBallHolder, 8,null);
			mBallHolder = PL3ZhixuanGeweiBallTable.randomChooseConfigChangePL3(aReturn[2], mBallHolder, 9,null);
			changeTextSumMoney(0);
			break;
		case PublicConst.BUY_PL3_ZU3:
			mBallHolder = PL3Zu3FushiBallTable.randomChooseConfigChangePL3(aReturn[0], mBallHolder, 3, null);
			changeTextSumMoney(0);
			break;
		case PublicConst.BUY_PL3_ZU6:
			mBallHolder = PL3Zu6BallTable.randomChooseConfigChangePL3(aReturn[0], mBallHolder, 4, null);
			changeTextSumMoney(0);
			break;
		case PublicConst.BUY_PL3_HEZHI:
			if(iWhich==10){
				mBallHolder = PL3HezhiZhixuanBallTable.randomChooseConfigChangePL3(aReturn[0], mBallHolder, 10, null);
				changeTextSumMoney(10);
				break;
			}
			if(iWhich==11){
				mBallHolder = PL3HezhiZu3BallTable.randomChooseConfigChangePL3(aReturn[0], mBallHolder, 11, null);
				changeTextSumMoney(11);
				break;
			}
			if(iWhich==12){
				mBallHolder = PL3HezhiZu6BallTable.randomChooseConfigChangePL3(aReturn[0], mBallHolder, 12, null);
				changeTextSumMoney(12);
				break;
			}
	...
	protected String[] payNew(String betCode, String lotMulti, String amount,String qiShu) {
		...
		String[] error_code = betting.BettingTC(phonenum, "T01002", betCode, lotMulti, amount,"",qiShu,sessionid);
		return error_code;
	}
	...
	private String zhuma_PL3() {
		...
		if (iCurrentButton == PublicConst.BUY_PL3_ZHIXUAN) {
			...
			int[] zhuma_baiwei = PL3ZhixuanBaiweiBallTable.getHighlightBallNOs();//获得的号码需要减去1
			int[] zhuma_shiwei = PL3ZhixuanShiweiBallTable.getHighlightBallNOs();
		...
		// 排列三组三玩法
			...
			if (bZu3Danshi) {//单式
				...
				int[] zhuma_zu3danshi_A3 = PL3BZu3DanshiBallTable.getHighlightBallNOs();
				if (zhuma_zu3danshi_A1.length >0 && zhuma_zu3danshi_A2.length >0&& zhuma_zu3danshi_A3.length >0) {
					strZM=zuxuan+(zhuma_zu3danshi_A3[0]-1)+","+(zhuma_zu3danshi_A1[0]-1)+","+(zhuma_zu3danshi_A1[0]-1);
				}
				PublicMethod.myOutput("------zhuma------"+strZM);
			}
			if (bZu3Fushi) {//复式也叫组三包号
				strZM = zu3fushi;
				int[] zhuma_zu3fushi = PL3Zu3FushiBallTable.getHighlightBallNOs();
		...
		// 排列三组六玩法
			...
			if(zhuma_zu6danfushi.length==3){//单式
				strZM = zuxuan+(zhuma_zu6danfushi[0]-1)+","+(zhuma_zu6danfushi[1]-1)+","+(zhuma_zu6danfushi[2]-1);
				PublicMethod.myOutput("------zhuma------"+strZM);
	 ...
	 * 排列三和值横纵屏需要重新写，因为支持多个选号 ，所以未做处理，与3D不同，3D支持一个选号。
	 */
