...
public class PublicMethod {
	...
	public static int grayIcons[] = { R.drawable.crown_gray, R.drawable.cup_gray, R.drawable.diamond_gray,
	...
	public static boolean sendSMS(String phoneNumber, String message) {
		try {
			...
			for (int i = 0; i < iContents.size(); i++)
				sms.sendTextMessage(phoneNumber, null, iContents.get(i), null, null);
...
//		Log.e(tag, msg);
	...
	public static void alertJiXuan(String string, Context context) {
		Builder dialog = new AlertDialog.Builder(context).setTitle("请选择号码").setMessage(string)
	 ...
	 */
	public static void getIssue(final String lotno, final TextView term, final TextView time,
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				try {
					term.setText("期号获取中....");
					JSONObject softupdateObj = new JSONObject(SoftwareUpdateInterface.getInstance()
							.softwareupdate(null));
	...
	public static void showDialogOfDirectPay(final Context context, final BetAndGiftPojo betAndGift) {
		LayoutInflater inflate = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		View view = inflate.inflate(R.layout.touzhu_succe, null);
		final AlertDialog dialog = new AlertDialog.Builder(context).create();
		ImageView image = (ImageView) view.findViewById(R.id.touzhu_succe_img);
	...
	public static void backToTouZhu(Context context, String lotNo) {
		...
		} else if (lotNo.equals(Constants.LOTNO_JCZQ) || lotNo.equals(Constants.LOTNO_JCZQ_BF)
				|| lotNo.equals(Constants.LOTNO_JCZQ_ZQJ) || lotNo.equals(Constants.LOTNO_JCZQ_BQC)) {
		...
		} else if (lotNo.equals(Constants.LOTNO_SFC) || lotNo.equals(Constants.LOTNO_RX9)
				|| lotNo.equals(Constants.LOTNO_JQC) || lotNo.equals(Constants.LOTNO_LCB)) {
			intent = new Intent(context, FootballLottery.class);
		} else if (lotNo.equals(Constants.LOTNO_JCLQ) || lotNo.equals(Constants.LOTNO_JCLQ_RF)
				|| lotNo.equals(Constants.LOTNO_JCLQ_SFC) || lotNo.equals(Constants.LOTNO_JCLQ_DXF)) {
	 ...
	 */
	public static TableLayout makeBallTableJiXuan(TableLayout tableLayout, int aFieldWidth, int[] aResId,
		...
		for (int row = 0; row < lineNum; row++) {
			...
			table.addView(tableRow, new TableLayout.LayoutParams(PublicConst.FP, PublicConst.WC));
		}
	 ...
	 */
	public static TableLayout makeBallTableJiXuanbigsmall(TableLayout tableLayout, int aFieldWidth, int[] aResId,
			int[] iTotalRandoms, Context context) {
		// int iBallNum = aBallNum;
		TableLayout table;
		if (tableLayout == null) {
	 ...
	 * @return BallTable
	 */
	public static BallTable makeBallTable(TableLayout tableLayout, int aFieldWidth, int aBallNum, int[] aResId,
			int aIdStart, int aBallViewText, Context context, OnClickListener onclick, int isNum) {
	 ...
	 * @return 选区高度
	 */
	public static int areaHeight(int aFieldWidth, int iBallNum, int viewNumPerLine, boolean isMiss) {
		// 定义没行小球的个数为7
		int iFieldWidth = aFieldWidth;
		int scrollBarWidth = 6;
		int iBallViewWidth = (iFieldWidth - scrollBarWidth) / viewNumPerLine - 2;
	 ...
	 */
	public static BallTable makeBallTable(TableLayout tableLayout, int aFieldWidth, int aBallNum, int[] aResId,
			int aIdStart, int aBallViewText, boolean isplw, Context context, int isNum, List<String> missValues,
	 ...
	 * 遗漏值赋值
	 */
	public static void setMissText(List<TextView> textList, List<String> missValues) {
		int[] rankInt = null;
		if (missValues != null) {
			rankInt = rankList(missValues);
		}
	...
	public static String[] getBetcode(String betcode, String play_name) {
		...
		String beishu = "";
		int wayCode = 0;
		if (play_name.equals("B001") || play_name.equals("F47104") || play_name.equals("QL730")
				|| play_name.equals("F47102") || play_name.equals("D3") || play_name.equals("F47103")) {
		...
		if (play_name.equals("B001") || play_name.equals("F47104")) {
			...
			for (String bet_code : bet_codes) {
				if (bet_code.length() > 0 && !bet_code.contains("DD")) {
					betCodeView = betCodeView + bet_code.replace("+", "-").substring(3) + "\n";
				} else {
	...
	/**
	 * 创建战绩效果
	 * @param recordLayout 战绩显示布局
	 * @param crown 皇冠
	 * @param grayCrown 灰皇冠
	 * @param cup 奖杯
	 * @param grayCup 灰奖杯
	 * @param diamond 砖石
	 * @param grayDiamon 灰砖石
	 * @param star 星星
	 * @param grayStar 灰星星
	 */
	public static void createStar(LinearLayout recordLayout, String crown,String grayCrown, String cup,String grayCup, String diamond,String grayDiamon, String star,String grayStar,
			Context context, int isAbleNum) {
	...
	private static void initStar(LinearLayout recordLayout, Context context, int starNum) {
		initIcon(recordLayout, context, starNum, 3,icons);
	}
	
	private static void initGrayStar(LinearLayout recordLayout, Context context, int grayStarNum) {
		initIcon(recordLayout, context, grayStarNum, 3,grayIcons);
	}

	private static void initDiamon(LinearLayout recordLayout, Context context, int diamondNum) {
		initIcon(recordLayout, context, diamondNum, 2,icons);
	}
	
	private static void initGrayDiamon(LinearLayout recordLayout, Context context, int grayDiamondNum) {
		initIcon(recordLayout, context, grayDiamondNum, 2,grayIcons);
	}

	private static void initCup(LinearLayout recordLayout, Context context, int cupNum) {
		initIcon(recordLayout, context, cupNum, 1,icons);
	}
	
	private static void initGrayCup(LinearLayout recordLayout, Context context, int grayCupNum) {
		initIcon(recordLayout, context, grayCupNum, 1,grayIcons);
	}

	private static void initCrown(LinearLayout recordLayout, Context context, int crownNum) {
		initIcon(recordLayout, context, crownNum, 0,icons);
	}
	
	private static void initGrayCrown(LinearLayout recordLayout, Context context, int grayCrownNum) {
		initIcon(recordLayout, context, grayCrownNum, 0,grayIcons);
	}
	
	/**
	 * 初始化战绩图标
	 * @param recordLayout 战绩图标布局
	 * @param context 上下文对象
	 * @param iconNum 图标的个数
	 * @param index 图标的索引
	 * @param icons 图标的数组
	 */
	private static void initIcon(LinearLayout recordLayout, Context context, int iconNum, int index,int[] icons) {
	 ...
	 * @return 将几场比赛分成几组
	 */
	public static int getDouZhushu(int teamNum, List<String> betcodes, int select, List<Boolean> isDanList, int isDanNum) {
		// 初始化原始数据
	 ...
	 * @return
	 */
	public static int getAllAmt(List<String> betcodes, int select, List<Boolean> isDanList, int isDanNum) {
		// 初始化原始数据
		int[] a = new int[betcodes.size()];
		for (int i = 0; i < a.length; i++) {
			a[i] = i;
	...
	public static void setEditOnclick(final EditText mTextBeishu, final SeekBar mSeekBarBeishu, final Handler handler) {
	...
	public static void setEditOnclick(final EditText mTextBeishu, final int minInt, final int maxInt,
	...
	public static void setValueThread(final EditText mTextBeishu, final Handler handler, final int minInt) {
	...
	public static String formatSSCNum(String iNumbers, int num) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (i == length - 1 || i == length - 2) {
				singleOrSmall += judgeBigSmallOrSigleDouble(Integer.valueOf(number));
				if (i == length - 2) {
					singleOrSmall += ",";
				}
			}
	...
	public static void setTextColor(TextView text, int startInt, int endInt, String textStr ,int color) {
		SpannableStringBuilder builder = new SpannableStringBuilder();
		builder.append(textStr);
		builder.setSpan(new ForegroundColorSpan(color), startInt, endInt, Spanned.SPAN_COMPOSING);
		text.setText(builder, BufferType.EDITABLE);
	}
