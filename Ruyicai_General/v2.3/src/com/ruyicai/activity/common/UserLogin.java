...
import com.ruyicai.net.transaction.LoginInterface;
import com.ruyicai.net.transaction.RegisterInterface;
...
public class UserLogin extends Activity implements TextWatcher {
	...
	final Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 2:
				progressDialog.dismiss();
				Toast.makeText(getBaseContext(), "用户已注册！", Toast.LENGTH_LONG).show();
				break;
			case 3:
				progressDialog.dismiss();
				...
				break;
			case 4:
				Toast.makeText(getBaseContext(), "该号被暂停请联系客服！",Toast.LENGTH_LONG).show();
				break;
			case 5:
				progressDialog.dismiss();
...
//		RuyicaiActivityManager.getInstance().addActivity(this);
	...
	private void beginLogin() {
		...
		if (editText.length() != 11) {
			...
			Thread t = new Thread(new Runnable() {
				public void run() {
					String error_code = LoginInterface.userlogin(String.valueOf(phoneNum_edit.getText()), password_string);
					if (error_code.equals("0000")) {
						...
						Log.e("phoneNum_edit.getText().toString()====", phoneNum_edit.getText().toString());
						Log.e("shellRW=====", ""+shellRW.getUserLoginInfo("phonenum"));
						b = true;
	...
	private void regToLogin() {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				String error_code = LoginInterface.userlogin(phonenum, password);
				if (error_code.equals("0000")) {
					String sessionid = LoginInterface.sessionid;
			...
			public void onClick(View v) {
				...
				if (register_phone_Text_string.length() != 11) {
					...
					Toast.makeText(getBaseContext(), "密码必须为6~15位！",
							Toast.LENGTH_LONG).show();
				} else if (!register_password_string
	...
	private void beginRegister() {
		...
		Thread regthread = new Thread(new Runnable() {
			...
			public void run() {
				...
				String error_code = RegisterInterface.getInstance().userregister(register_phone_num, register_password_, register_id, realName);
