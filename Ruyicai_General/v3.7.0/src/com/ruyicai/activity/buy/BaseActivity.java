...
public abstract class BaseActivity extends Activity implements OnClickListener{
	...
	public void stopSensor() {
		if(areaNums != null){
			for(int i=0;i<areaNums.length;i++){
				if(areaNums[i].jixuanBtn!=null&&areaNums[i].isSensor&&isJixuan==true){
					 sensor.stopAction();                          
				}
			}
		}
			...
			if(areaNums != null){
				if(areaNums[0].isAgain){
					...
					for(int i=0;i<areaNums.length;i++){
						...
						areaNums[i].table.changeBallState(areaNums[i].chosenBallSum, iHighlightBallId[i]);
	...
	public int[] getRandom(int aRandomNums) {
		int[] iHighlightBallId = PublicMethod.getRandomsWithoutCollision(aRandomNums, 0, areaNums[0].areaNum - 1);
