...
public class Hemaidetail extends Activity implements HandlerMsg {
	...
	public void leavTextView(TextView text, boolean isRen) {
		...
		String renAmt = leavMount(detatil.getRemainderAmt(), amountEdit.getText()
				.toString());
	...
	public void setValue(JoinDetatil detatil) {
		...
		contentListView.createListContent(layoutMain, content,
				detatil.getLotNo(), detatil.getBetCodeHtml(), detatil.getBetCodeJson());
		amountProgress.setText("占总额"
				+ progress(amountEdit.getText().toString(), detatil.getTotalAmt())
				+ "%");
		safeProgress.setText("占总额"
				+ progress(safeAmtEdit.getText().toString(), detatil.getTotalAmt())
				+ "%");
		showMinText(detatil.getMinAmt());
		leavTextView(amountText, true);
		leavTextView(safeText, false);
		PublicMethod.createStar(starLayout, String.valueOf(detatil.getCrown()),
				String.valueOf(detatil.getGraycrown()), String.valueOf(detatil.getCup()), String.valueOf(detatil.getGraycup()),
				String.valueOf(detatil.getDiamond()), String.valueOf(detatil.getGraydiamond()),
				String.valueOf(detatil.getGoldStar()), String.valueOf(detatil.getGraygoldStar()),
				Hemaidetail.this, 0);
	...
	public void joinCanyuNet() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				String str = "00";
				str = QueryJoinCanyuInterface.queryLotJoincanyu(caseLotoId, userno,
	...
	public void joinCannelNet() {
		...
		Thread t = new Thread(new Runnable() {
			...
			@Override
			public void run() {
				str = JoinCannelInterface.joincanel(caseLotoId, userno, phonenum);
				try {
					json = new JSONObject(str);
	...
	public void errorCode_0000() {
		if (isJoinIn) {
			...
			setValue(getValue());
			amountProgress.setText("占总额"
					+ progress(amountEdit.getText().toString(), detatil.getBuyProgress())
					+ "%");
			safeProgress.setText("占总额"
					+ progress(safeAmtEdit.getText().toString(), detatil.getSafeProgress())
					+ "%");
			leavTextView(amountText, true);
			leavTextView(safeText, false);
			// linshi
			PublicMethod.createStar(starLayout, String.valueOf(detatil.getCrown()),
					String.valueOf(detatil.getGraycrown()), String.valueOf(detatil.getCup()), String.valueOf(detatil.getGraycup()),
					String.valueOf(detatil.getDiamond()), String.valueOf(detatil.getGraydiamond()),
					String.valueOf(detatil.getGoldStar()), String.valueOf(detatil.getGraygoldStar()), Hemaidetail.this,
					0);
		}
	}
