...
public class BetQueryInterface {
	...
	/**
	 * 投注查询的方法
	 * 	@param betQueryPojo  下面是 betQueryPojo中的变量
	 * 	@param   userno	用户编号	登录成功返回的用户编号如00000001<br>
	 *	@param  startline	开始条数	开始条数<br>
	 *	@param  stopline	截止条数	截止条数<br>
	 *	@param  lotno	(预留的可以传空)彩种	彩种（可以传空预留）<br>
	 *	@param  sessionid	sessionid<br>	
	 *	@param  batchcode	期号	期号<br>
	 *	@param  type	查询类型	type  投注查询为bet<br>
	 *	@param  command	请求	command:QueryLot<br>
	 * @return
	...
	public String betQuery(BetAndWinAndTrackAndGiftQueryPojo betQueryPojo){
		...
		try {
			...
			jsonProtocol.put(ProtocolManager.REGUESTTYPE, betQueryPojo.getType());
