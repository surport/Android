...
public class PrizeNotificationService extends Service {
	...
	public void onStart(Intent intent, int startId) {
		...
		Log.v("onStart","=====");
		Log.v("batchCodeValue",""+batchCodeValue);
		Log.v("batchCodeValue.length",""+batchCodeValue.length);
		Log.v("listPrizeNew.size()",""+listPrizeNew.size());
		Log.v("listPrize.size()",""+listPrize.size());
...
//		Collections.copy(listPrize, listPrizeNew);
	...
	private void lotteryShow(boolean[] lotteryIShow) {
		...
		for(int i = 0; i<lotteryIShow.length;i++){
			...
			if(lotteryIShow[i] == true&&shellRW.getBoolean(Constants.orderPrize[i]) == true){
	...
	Object[] netting() throws SocketException{
    	JSONObject prizeReturn = NoticeWinInterface.getInstance().getLotteryAllNotice();
    	Log.v("prizeReturn",""+prizeReturn);
    	List<Map<String,Object>> listPrizeNet = new ArrayList<Map<String,Object>>();
    	int[] batchCodeValueNet = new int[lotteryLable.length];
    	/*联网失败，默认返回{error_code:0,massage:联网异常}
    	 *这是对联网失败的处理
    	 **/
    	String errorcode;
    	try{
    	...
    	if(errorcode.equals("0000")){
    		try {
   			 nextNetTime = Integer.valueOf(prizeReturn.getString("noticeTime"));
			} catch (JSONException e) {
   ...
   /**
    * 根据彩种和相关数据弹出通知
    * @param lotteryName 要弹出的彩种名
    * @param map  带有开奖号码和开奖期号的Map
    */
    private void showNotification(int i,String lotteryName,Map<String,Object> map) {
    	Log.v("showNotification",""+lotteryName);
    	CharSequence text  = lotteryName + "第" + map.get("batchCode")+"期开奖号码为：";
    	CharSequence text2  = lotteryName + "第" + map.get("batchCode")+"期";
    	CharSequence text3  = "开奖号码为："+map.get("winCode");
        notification = new Notification(R.drawable.notification, text, System.currentTimeMillis());
        notification.flags |= Notification.FLAG_AUTO_CANCEL;
        RemoteViews customNoti = new RemoteViews(getPackageName(), R.layout.notification_orderprize);
