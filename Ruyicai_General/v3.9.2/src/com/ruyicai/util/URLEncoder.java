...
/**
 * Utility class for form encoding.this class is modified form
 * java.net.URLEncoder so that it can work well in cldc env. This class contains
 ...
 * ü@foo-bar" would get converted to "The+string+%C3%BC%40foo-bar" because in
 * UTF-8 the character ü is encoded as two bytes C3 (hex) and BC (hex), and the
...
public class URLEncoder {
	...
	static {
		...
		dontNeedEncoding[' '] = true; // encoding a space to a + is done in the
										// encode() method
	 ...
	 * @param enc
	 *            The name of a supported character encoding such as UTF-8
	...
	public static String encode(String s, String enc) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			// PublicMethod.myOutput("Examining character: " + c);
			if (c < 256 && dontNeedEncoding[c]) {
				if (c == ' ') {
					c = '+';
					needToChange = true;
				}
				// PublicMethod.myOutput("Storing: " + c);
				out.append((char) c);
				...
				try {
					...
					if (c >= 0xD800 && c <= 0xDBFF) {
						/*
						 * PublicMethod.myOutput(Integer.toHexString(c) +
						 * " is high surrogate");
						 */
						if ((i + 1) < s.length()) {
							int d = (int) s.charAt(i + 1);
							/*
							 * PublicMethod.myOutput("\tExamining " +
	...
	public static String decode(String url) {
		...
		try {
			...
			String text = UTF8Decode(data, 0, data.length);
			return text;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
