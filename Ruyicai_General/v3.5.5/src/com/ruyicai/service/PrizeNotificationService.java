...
public class PrizeNotificationService extends Service {
	...
	int  nextNetTime;
	String[] lotteryLable = {"ssq","ddd","qlc","dlt","qxc","pl3", "pl5","22-5"};//从开奖json串中获取响应数据的标签
...
//		Collections.copy(listPrize, listPrizeNew);
	...
	private void lotteryShow(boolean[] lotteryIShow) {
		...
		for(int i = 0; i<lotteryIShow.length;i++){
			...
			if(lotteryIShow[i] == true&&shellRW.getBooleanValue(Constants.orderPrize[i]) == true){
    ...
    */
    private void showNotification(int i,String lotteryName,Map<String,Object> map) {
    	...
    	CharSequence text3  = "开奖号码为："+FormatZhuma.formatZhuma(i,map.get("winCode").toString());
