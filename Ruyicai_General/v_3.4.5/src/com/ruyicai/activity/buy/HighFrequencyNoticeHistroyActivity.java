		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String msg = "" ;
				 String code = "";
				do {
					final JSONObject prizemore = PrizeInfoInterface	.getInstance().getNoticePrizeInfo(lotno, "1", "100");
					if (prizemore == null) {
					...
					try {
						...
						if (code.equals("0000")) {
							...
							tHandler.post(new Runnable() {
								@Override
								public void run() {
									SubEfficientAdapter adapter = new SubEfficientAdapter(	HighFrequencyNoticeHistroyActivity.this,	str, list);
				...
				tHandler.post(new Runnable() {
					public void run() {
						dialog.dismiss();
						Toast.makeText(HighFrequencyNoticeHistroyActivity.this,message, Toast.LENGTH_SHORT).show();
					}
				});
			}
		}).start();
