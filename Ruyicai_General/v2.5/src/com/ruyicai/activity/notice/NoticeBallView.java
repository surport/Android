...
public class NoticeBallView extends View{
     ...
     * @param list 彩种信息的数组
     */
	public void init(int line,int row,int startNum,List<JSONObject> list,boolean isRed,String lotno,double release){
		iGameType = lotno;
		this.isRed = isRed;
		this.line = line;
		this.row = row;
			...
			for(int j=0;j<3;j++){
				for(int i=0;i<row;i++){
					if(j==0){
						...
						canvas.drawBitmap(bitNoticeYellow, FIRST_WITH+i*WITH+(row*WITH*j), 0, null);
					}
...
//				canvas.drawBitmap(getBitmapFromRes(res,FIRST_WITH,WITH), 0, WITH+i*WITH, null);
	 ...
	 * 福彩3d小球
	 */
	public void onDrawCenterPL3(Canvas canvas){
      for(int m=0;m<3;m++){
