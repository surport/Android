...
public class LuckChoose extends Activity implements HandlerMsg{
	// 幸运选号
	private static final String[] chooseLuckLotteryNum_title = { "双色球", "福彩3D","七乐彩", "排列三", "大乐透","排列五","七星彩"};
	private static final Integer[] mIcon = { R.drawable.join_ssq,R.drawable.join_fc3d, R.drawable.join_qlc, R.drawable.join_pl3,R.drawable.join_dlt,R.drawable.join_pl5,R.drawable.join_qxc}; // zlm 8.9 添加排列三、超级大乐透图标
...
//	ReturnPage returnPage;
	...
	public class ChooseLuckLotteryNum_EfficientAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// 与布局中的信息关联起来
			if (convertView == null) {
				convertView = mInflater.inflate(R.layout.choose_luck_lottery_num_listview_layout_two, null);
	...
	private void dialogLuck(String aGameType) {
		...
		}else if (aGameType.equalsIgnoreCase("pl5")) {
			title = getResources().getString(R.string.xyxh_pl5);
		}else if (aGameType.equalsIgnoreCase("qxc")) {
			title = getResources().getString(R.string.xyxh_qxc);
		}
	...
	public int[] showAllBallLayout(String aGameType, LinearLayout layout) {
		...
		if (aGameType.equalsIgnoreCase("ssq")) {
			...
			int[] group = new int[5];
			int[] group01 = new int[7];
			group01 = getBallNum(aGameType, 5);
			for (int i = 0; i < 5; i++) {
				group[i] = group01[i];
			}
			// int[] randomNumGroup ;
	...
	public int[] getBallNum(String aGameType, int aRandomNums) {
		...
		if (aGameType.equalsIgnoreCase("ssq")) {
		...
		}else if (aGameType.equalsIgnoreCase("pl5")) { // zlm 排列五
			int[] iShowBallNum01 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum02 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum03 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum04 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum05 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			ballNumStr[0] = iShowBallNum01[0];
			ballNumStr[1] = iShowBallNum02[0];
			ballNumStr[2] = iShowBallNum03[0];
			ballNumStr[3] = iShowBallNum04[0];
			ballNumStr[4] = iShowBallNum05[0];
		}else if (aGameType.equalsIgnoreCase("qxc")) { // zlm 七星彩
			int[] iShowBallNum01 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum02 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			...
			int[] iShowBallNum05 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum06 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			int[] iShowBallNum07 = PublicMethod.getRandomsWithoutCollision(
					aRandomNums - 2, 0, 9);
			ballNumStr[0] = iShowBallNum01[0];
			ballNumStr[1] = iShowBallNum02[0];
			ballNumStr[2] = iShowBallNum03[0];
			ballNumStr[3] = iShowBallNum04[0];
			ballNumStr[4] = iShowBallNum05[0];
			ballNumStr[5] = iShowBallNum06[0];
			ballNumStr[6] = iShowBallNum07[0];
		}
	...
	public void gameClassify(View v) {
		if (Math.floor(v.getId() / 4) == 25) {
			...
			type02 = "cjdlt";
		} else if (Math.floor(v.getId() / 4) == 30) { // 排列五
			type02 = "pl5";
		} else if (Math.floor(v.getId() / 4) == 31) { // 七星彩
			type02 = "qxc";
		}
	}
	/**
	...
	protected void showAgreeAndPayDialog(String aGameType) {
		...
		String[] gameType = { "ssq", "fc3d", "qlc", "pl3", "cjdlt","pl5","qxc" };
		int[] titleID = { R.string.shuangseqiu, R.string.fucai3d,R.string.qilecai, R.string.pailiesan, R.string.chaojidaletou,R.string.pailie5,R.string.qxc};
		LayoutInflater inflater = LayoutInflater.from(this);
	...
	public void showAgreeAndPayBallLayout(String aGameType,
		...
		} else if (aGameType.equalsIgnoreCase("qlc")||aGameType.equalsIgnoreCase("qxc")) {
			OneBallView showBallView;
			for (int i = 0; i < 7; i++) {
				showBallView = new OneBallView(this);
				showBallView.initBall(BALL_WIDTH, BALL_WIDTH, ""
	...
	public int[][] changeShuZu(int jiZhu, String aGameType) {
		...
		if (aGameType.equalsIgnoreCase("ssq")
				...
				|| aGameType.equalsIgnoreCase("cjdlt")||aGameType.equalsIgnoreCase("qxc")) {
			switch (jiZhu) {
			case 1:
				zhuShu = new int[1][7];
