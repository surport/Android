 ...
 *  "news":"提示信息"
 *  "currentBatchCode":"{"F47102"：{当前期号,结束时间}}"
 *  }
 */
public class SoftwareUpdateInterface {
	...
	public String softwareupdate(JSONObject statInfo) {
		...
		try {
			...
			Log.e("HomeActivity=======", "========="+Constants.SERVER_URL+"|||"+Constants.LOT_SERVER);
	 ...
	 * statInfo 统计信息
	 */
	public String softwareupdate(JSONObject statInfo,String randomNumber) {
		String reValue = "";
		try {
			JSONObject jsonProtocol = ProtocolManager.getInstance().getDefaultJsonProtocol();
			jsonProtocol.put(ProtocolManager.COMMAND, COMMAND);
