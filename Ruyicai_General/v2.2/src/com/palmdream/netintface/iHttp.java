...
public class iHttp {
	...
	private void setRequestHeaders(HttpURLConnection con) {
		try {
			 ...
			 * PublicMethod.myOutput("----- "+prof); int space =
			 * prof.indexOf(" ");
			 * 
			 * if (space != -1) { prof = prof.substring(0, space - 1); }
			 * 
			...
			// con.setRequestProperty("User_Agent",
			// "Android Application:"+Z.APP_VERSION);
			con.setRequestProperty("User-Agent", ua);
			/*
			 * if (locale != null) { con.setRequestProperty("Content-Language",
			 * locale); }
			 */
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			if (conMethord == CMWAP) {
				...
				if (iHttp.conMethord == iHttp.CMUNKNOWN) {
					iHttpAuto_CMWAP autoCheckWAP = new iHttpAuto_CMWAP();
					PublicMethod
							.myOutput("---------iHttp.conMethord == iHttp.CMUNKNOWN");
					autoCheckWAP.setUrl(url);
					autoCheckWAP.run();
				}
	...
	public String getViaHttpConnection_(String url) {

		String reValue = "";
		try {
			if (conMethord == CMWAP) {
				PublicMethod.myOutput("----------getViaHttpConnection cmwap");// ("tag","----------getViaHttpConnection cmwap");
				reValue = getViaHttpConnection_CMWAP_(url, false);
			}

			else if (conMethord == CMNET) {
				PublicMethod.myOutput("----------getViaHttpConnection cmnet"
						+ url);// Log.d("tag","----------getViaHttpConnection cmnet");
				reValue = getViaHttpConnection_CMNET_(url);
			} else {
				PublicMethod.myOutput("----------getViaHttpConnection else");
	...
	public String getViaHttpConnection_CMNET(String url) {
			...
			if (this.CMNET_USE_HTTPS) {
				...
				try {
					...
					while (true) {
						...
						Log
								.d("tag",
										"-------getViaHttpConnection_CMNET(String url)");
					...
					e.printStackTrace();
					// iHttp.conMethord=1;
					PublicMethod.myOutput("----------iHttp.conMethord"
							+ iHttp.conMethord);
				} finally {
					if (is != null) {
						try {
		...
		PublicMethod.myOutput("-------getViaHttpConnection_CMNET-----"
				+ reValue);
		return reValue;

	}

	public String getViaHttpConnection_CMNET_(String url) {
		HttpURLConnection con = null;
		InputStream is = null;
		OutputStream os = null;
		String reValue = "";
			...
			if (!this.CMNET_USE_HTTPS) {
				...
				try {
					...
					while (true) {
						...
						status = con.getResponseCode();
						if (/*
							 * (status == HttpsURLConnection.HTTP_TEMP_REDIRECT)
							 * ||
	...
	public String getViaHttpConnection_CMWAP(String url, boolean check) {
			...
			if (this.CMWAP_USE_HTTPS) {
				...
				try {
					...
					if ((status == HttpURLConnection.HTTP_NOT_IMPLEMENTED)
						...
						String str = GetStringFromInputStream(is);
						PublicMethod
								.myOutput("------------GetStringFromInputStream(is):"
										+ str);
						is = GetInputStreamFromString(str);
						buffer.setLength(0);
		...
		PublicMethod.myOutput("-----------------getViaHttpConnection_CMWAP"
				+ reValue);
		return reValue;
	}

	public String getViaHttpConnection_CMWAP_(String url, boolean check) {
		HttpURLConnection con = null;
		InputStream is = null;
		OutputStream os = null;
		String reValue = "";
				...
				try {
					...
					while (true) {
						...
						if (proxy != null && url.length() > 7
							...
							Log
									.d("tag",
											"-------getViaHttpsConnection_CMWAP(String url,boolean check)");
							con.setDoInput(true);
							con.setConnectTimeout(iTimeOut);
							String requestProperty = (ind >= 0) ? url
									.substring(8, ind) : url.substring(8);
							con.setRequestProperty("X-Online-Host",
									(ind >= 0) ? url.substring(8, ind) : url
											.substring(8));
						...
						if (// (status ==
						// HttpsURLConnection.HTTP_TEMP_REDIRECT)||
						(status == HttpsURLConnection.HTTP_MOVED_TEMP)
								|| (status == HttpsURLConnection.HTTP_MOVED_PERM)) {
							// Get the new location and close the connection
							url = con.getHeaderField("location");
