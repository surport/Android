...
public class BettingInterface {
	...
	public String Betting(String login_User, String bets, String bets_zhu_num,
			String sessionid) {

		JSONObject obj = null;
		while (iretry < 3 && iretry > 0) {
			try {
				String re = jrtLot.betting(login_User, bets, Integer
						.parseInt(bets_zhu_num)
						* 200 + "", sessionid);
		...
		if (iretry == 0 && iHttp.whetherChange == false) {
			...
			Betting(login_User, bets, bets_zhu_num, sessionid);
		}
		return error_code;
	...
	public String BettingNew(String bet_code, String count, String amount,
		...
		if (iretry == 0 && iHttp.whetherChange == false) {
			...
			iretry = 2;
			BettingNew(bet_code, count, amount, sessionid);
		}
		return error_code;
	...
	public String[] BettingTC(String mobileCode, String lotNo, String betCode,
		...
		while (iretry < 3 && iretry > 0) {
			try {
				// String re = jrtLot.addNumQueryTC(sessionid, mobileCode,
				// startLine, stopLine)(mobileCode, lotNo, betCode, lotMulti,
				// amount+"00", oneMoney,batchnum,sessionid);
				String re = jrtLot.bettingTC(mobileCode, lotNo, betCode,
				...
				PublicMethod.myOutput("---------" + error_code
						+ "-----error_code_DYJ--" + error_code_DYJ);
				e.printStackTrace();
				if (error_code.equalsIgnoreCase("00")
						&& error_code_DYJ.equalsIgnoreCase("00")) {
					iretry--;
				} else {
					iretry = 3;
				}
		...
		if (iretry == 0 && iHttp.whetherChange == false) {
			...
			iretry = 2;
			BettingTC(mobileCode, lotNo, betCode, lotMulti, amount, oneMoney,
					batchnum, sessionid);
		}
		iretry = 2;
