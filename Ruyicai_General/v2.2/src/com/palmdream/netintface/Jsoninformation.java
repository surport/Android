...
public class Jsoninformation {
	...
	static String getlotno_sfc;
	static String getlotno_rxq;
	static String getlotno_jqc;
	static String getlotno_lcb;
	...
	public static String[] getjsoninformation() {
		...
		if (tnotice.equals("")) {
			...
			getlotno_sfc = "getlotno_sfc";
			getlotno_rxq = "getlotno_rxq";
			getlotno_jqc = "getlotno_jqc";
			getlotno_lcb = "getlotno_lcb";
			...
			getlotno_sfc = getLotNo_zc("T01003");
			getlotno_jqc = getLotNo_zc("T01005");
			getlotno_lcb = getLotNo_zc("T01006");
			getlotno_rxq = getLotNo_zc("T01004");
		...
		String[] str = { tnotice, ssqinfo, dddinfo, qlcinfo, getlotno_ssq,
				getlotno_ddd, getlotno_qlc, dltinfo, pl3info, getlotno_dlt,
				getlotno_pl3, getlotno_sfc, getlotno_rxq, getlotno_jqc,
				getlotno_lcb };//
		// str=str1;
			...
			if (!re.equalsIgnoreCase("")) {
				try {
					try {
						...
						PublicMethod.myOutput("===ssqerrorcode===="
								+ error_code);
					} catch (Exception e) {
			...
			if (!re.equalsIgnoreCase("")) {
				try {
					try {
						JSONObject obj = new JSONObject(re);
						error_code = obj.getString("error_code");
						PublicMethod.myOutput("===ddderrorcode===="
								+ error_code);
					} catch (Exception e) {
						JSONArray jsonObject3 = new JSONArray(re);
						JSONObject obj = jsonObject3.getJSONObject(0);
			...
			if (!re.equalsIgnoreCase("")) {
				try {
					try {
						JSONObject obj = new JSONObject(re);
						error_code = obj.getString("error_code");
						PublicMethod.myOutput("===qlcerrorcode===="
								+ error_code);
					} catch (Exception e) {
						JSONArray jsonObject3 = new JSONArray(re);
						JSONObject obj = jsonObject3.getJSONObject(0);
	...
	// 获取时时彩彩开奖信息的后台返回值
	// 获取大乐透开奖信息的后台返回值
	public static String sscinfo() {
		String re = "";
		String error_code;
		while (iretrytimes < 3 && iretrytimes > 0) {
			re = jrtLot.tlotterySelect("T01007");// F47102
			if (!re.equalsIgnoreCase("")) {
				try {
					try {
						JSONObject obj = new JSONObject(re);
	...
	// 获取时时彩彩开奖信息的后台返回值
	// 获取大乐透开奖信息的后台返回值
	public static String zcinfo(String lotNo) {
		String re = "";
		String error_code;
		while (iretrytimes < 3 && iretrytimes > 0) {
			re = jrtLot.tlotterySelect(lotNo);// F47102
			if (!re.equalsIgnoreCase("")) {
				try {
					try {
						JSONObject obj = new JSONObject(re);
	...
	// 获取足彩当前期号的后台返回值
	public static String getLotNo_zc(String lotno) {
		String re = "";
		String batchCode;
		// while (iretrytimes < 3 && iretrytimes > 0) {
		// re = jrtLot.getZCLotNo(lotno);
		// if (!re.equalsIgnoreCase("")) {
		// try {
		// JSONObject obj = new JSONObject(re);
		// batchCode = obj.getString("batchCode");
