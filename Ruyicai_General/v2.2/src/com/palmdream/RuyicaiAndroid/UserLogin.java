...
public class UserLogin extends Activity implements TextWatcher {
		...
		Thread regthread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (error_code.equals("0000")) {
					...
					userLogon(register_phone_num);// 注册统计
					Message msg = new Message();
					msg.what = 1;
					handler.sendMessage(msg);
	...
	public void userLogin() {
		...
		new Thread(new Runnable() {
			...
			@Override
			public void run() {
				jrtLot.count("2", HomePage.imei, null, null);
				jrtLot.addUserLogData(HomePage.channel_id, "C", shellRW.getUserLoginInfo("sessionid"), 
						shellRW.getUserLoginInfo("phonenum"));
			}
		}).start();
	}
	 ...
	 * @版本：
	 */
	public void userLogon(final String phonenum) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				jrtLot.count("3", HomePage.imei, null, phonenum);
			}
		}).start();
	}
}
