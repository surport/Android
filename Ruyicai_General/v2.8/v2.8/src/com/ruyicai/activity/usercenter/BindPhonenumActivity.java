	...
	Handler handler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case 1://提交绑定手机号成功
				...
				Toast.makeText(BindPhonenumActivity.this, (String) msg.obj, Toast.LENGTH_LONG).show();
				initSubmitBindSecurtyCode();
				break;
			case 2://提交验证码
				progressDialog.dismiss();
				Toast.makeText(BindPhonenumActivity.this, (String) msg.obj, Toast.LENGTH_LONG).show();
				shellRW.setUserLoginInfo("mobileid", bindphonenum);
				finish();
				break;
			case 3://提交绑定手机号失败
				progressDialog.dismiss();
		...
		submit.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if(bindphonenum.length()<11){
					Toast.makeText(BindPhonenumActivity.this, "请输入正确的手机号格式", Toast.LENGTH_SHORT).show();
				}else{
					showDialog(0);
					new Thread(new Runnable() {
						public void run() {
							String bindIDback = BindPhoneInterface.getInstance().submitPhonenum(bindphonenum, phonenum, userno);
								try {
		...
		submit.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if(securityCode.length()==0){
					Toast.makeText(BindPhonenumActivity.this, "验证码不能为空", Toast.LENGTH_SHORT).show();
				}else{
					showDialog(0);
					new Thread(new Runnable() {
						public void run() {
							String bindIDback = BindPhoneInterface.getInstance().submitSecurityCode(securityCode,bindphonenum, phonenum, userno);
								try {
									JSONObject bindIDjson = new JSONObject(bindIDback);
									String errorCode = bindIDjson.getString("error_code");
									String message = bindIDjson.getString("message");
