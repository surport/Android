...
public abstract class HttpHeaderFactory {
	...
	public String getWeiboAuthHeader(String method, String url,
		...
		// step 2: authParams有两个用处：1.加密串一部分 2.生成暿Authorization头域
		WeiboParameters authParams = this.generateAuthParameters(nonce,
				timestamp, token);
		// 生成用于计算signature的，参数䶿
		WeiboParameters signatureParams = this.generateSignatureParameters(
				authParams, params, url);
		// step 3: 生成用于签名的base String
		String oauthBaseString = this.generateAuthSignature(method,
				signatureParams, url, token);
	...
	private String generateAuthSignature(final String method,
			WeiboParameters signatureParams, final String url, Token token) {
	...
	private WeiboParameters generateSignatureParameters(
			WeiboParameters authParams, WeiboParameters params, String url) {
	...
	private WeiboParameters generateAuthParameters(long nonce, long timestamp,
			Token token) {
		...
		authParams.add("oauth_signature_method",
				HttpHeaderFactory.CONST_SIGNATURE_METHOD);
	...
	// 生成用于哈希的base string串，注意要按顺序，按隿档需求参数生成，否则40107错误
	...
	// params
	public abstract void addAdditionalParams(WeiboParameters des,
			WeiboParameters src);

	// 解析url中参数对,存储到signatureBaseParams
	public void parseUrlParameters(String url,
			WeiboParameters signatureBaseParams) {
	...
	public abstract String generateSignature(String data, Token token);
	...
	public static String encodeParameters(Bundle postParams, String split,
			boolean quot) {
