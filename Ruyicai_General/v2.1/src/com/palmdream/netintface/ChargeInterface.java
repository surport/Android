...
public class ChargeInterface {
	...
	public String phonebankcharge(String accesstype, String mobile_code,
		...
		while (iretrytimes < 3 && iretrytimes > 0) {
			re = jrtLot.wapAttemperRequest(accesstype, mobile_code, cardType,
		...
		PublicMethod.myOutput("---------iHttp.whetherChange----------"
				+ iHttp.whetherChange);
		// 陈晨 8.11 接 入点改变判断
		if (iretrytimes == 0 && iHttp.whetherChange == false) {
			...
			iretrytimes = 2;
			phonebankcharge(accesstype, mobile_code, cardType,
					transaction_money, bankId, expand, sessionid);
		}
		return error_code;
	}
	...
	public String phonecardcharge(String accesstype, String mobile_code,
			...
			String sessionid) {
		while (iretrytimes < 3 && iretrytimes > 0) {
			re = jrtLot.attemperRequest(accesstype, mobile_code, cardType,
					transaction_money + "00", totalAmount + "00", card_no,
		...
		if (iretrytimes == 0 && iHttp.whetherChange == false) {
			...
			iretrytimes = 2;
			phonecardcharge(accesstype, mobile_code, cardType,
					transaction_money, totalAmount, card_no, card_pwd, bankId,
					expand, sessionid);
		}
		return error_code;
	}
