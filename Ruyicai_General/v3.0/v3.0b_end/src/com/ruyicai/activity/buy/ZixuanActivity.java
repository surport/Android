...
public class  ZixuanActivity extends Activity implements OnClickListener,SeekBar.OnSeekBarChangeListener,HandlerMsg{
	 ...
	 * isTen 显示的注码小于十的是否加零
	 */
	public void createView(AreaInfo areaInfos[],CodeInterface code,BuyImplement buyImplement,boolean isTen) {
		this.isTen = isTen;
		if(areaNums!=null){
		...
		for(int i=0;i<areaNums.length;i++){
			...
			areaNums[i].table =  makeBallTable(areaNums[i].tableLayout, iScreenWidth, info.areaNum,info.ballResId, info.aIdStart, info.aBallViewText,isTen ,this, this);
			areaNums[i].init();
		}		
	 ...
	 * @return BallTable
	 */
	public BallTable makeBallTable(TableLayout tableLayout, int aFieldWidth,int aBallNum, int[] aResId, int aIdStart, int aBallViewText, boolean isTen,
	 ...
	 * @return
	 */
	public void alert(final String zhuma) {
	 ...
	 */
	public void initBet(){
		betAndGift.setAmount(""+iZhuShu*iProgressBeishu*betAndGift.getAmt()*100);
    ...
    */
	...
	public String getTouzhuAlert(){
		...
		return "注数："
				+ iZhuShu + "注     " 
				+ "金额：" + iZhuShu * betAndGift.getAmt() *iProgressBeishu+ "元" 
				+ "(冻结金额："
				+ (betAndGift.getAmt() * (iProgressQishu - 1) * iZhuShu*iProgressBeishu)
				+ "元)"; 
	}
	/**
	 * 提示框1 用来提醒选球规则
