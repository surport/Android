...
public class NoticeDataProvider {
	...
	protected static List<Map<String, Object>> getListForMainListViewSimpleAdapter() {
		...
		String iGameName[] = {"ssq", "fc3d", "qlc",  
				"cjdlt","pl3","pl5","qxc", "ssc", "11-5", "11-ydj","22-5","sfc", "rxj", "lcb", "jqc","jcz","jcl"}; // 8.9
		// 添加：排列三、超级大乐透
		...
		try {
			// 添加：进球彩
			map = new HashMap<String, Object>();
			map.put(LOTTERYTYPE, iGameName[14]);
...
//			e.printStackTrace();
		...
		try {
			// 添加：竞彩足球
			map = new HashMap<String, Object>();
			map.put(LOTTERYTYPE, iGameName[15]);
			list.add(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			// 添加：竞彩篮球
			map = new HashMap<String, Object>();
			map.put(LOTTERYTYPE, iGameName[16]);
			list.add(map);
		} catch (Exception e) {
			e.printStackTrace();
		}
