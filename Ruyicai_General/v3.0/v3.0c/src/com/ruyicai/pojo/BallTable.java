...
public class BallTable {
	...
	public String[] getHighlightStr() {
		...
		for (i = 0; i < iSum; i++) {
			if (getOneBallStatue(i) == 1) {
				iBallNOs[iCurrent] = ""+i + 1;
				iBallNOs[iCurrent] = ((OneBallView) ballViewVector.elementAt(i)).getNum();
				iCurrent++;
			}
		}
		return iBallNOs;
	...
	/**
	 * 用于福彩3D横纵屏切换
	 * @param aRandomNums     允许高亮小球的个数
	 * @param mBallHolder     记录当前页面各个控件状态的累
	 * @param whichGroupBall  是哪个BallTable
	 * @param randomNumbers   传的随机数（为组三单式玩法、胆拖玩法、和值玩法准备）
	 * @return
	 */
