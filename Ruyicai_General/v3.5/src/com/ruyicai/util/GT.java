...
public class GT {
	 ...
	 * 将投注码生成字符串
	 * 
	 * @param caipiaoIdIndex: 彩票类型 0 —— 双色球; 1 —— 福彩3D; 2 —— 七乐彩
	 * @param caipiaoWanfaIndex: 高频彩的玩法类型
	 * @param maxNum: 最大投注数
	 * @param playType: 彩票玩法
	 * @param mul: 整型投注码的倍数
	 * @param betCode: 整型投注码
	 * @return
	 */
	public static String betCodeToString(int caipiaoIdIndex, int caipiaoWanfaIndex, int maxNum, String playType, int mul, int[][] betCode) {
		...
		case 0:
			if (playType.equals("00")) {
				for (int i = 0; i < betCode.length; i++) {
					...
					for (int j = 0; j < 6; j++) {
						if (betCode[i][j] < 10) {
							strBetCode .append("0" + betCode[i][j]);
						} else {
							strBetCode.append(betCode[i][j]);
						}
					}
					strBetCode.append("~");
					if (betCode[i][6] < 10) {
						strBetCode.append("0" + betCode[i][6]);
					} else {
						strBetCode.append(betCode[i][6]);
					}
					strBetCode.append("^_01_200_200");
		...
		case 1:
			for (int i = 0; i < betCode.length; i++) {
				...
				if (betCode[i][betCode[i].length - 3] < 10) {
					strBetCode.append("0" + betCode[i][betCode[i].length - 3] + "-");
				} else {
					strBetCode.append(betCode[i][betCode[i].length - 3] + "-");
				}
				if (betCode[i][betCode[i].length - 2] < 10) {
					strBetCode.append("0" + betCode[i][betCode[i].length - 2] + " ");
				} else {
					strBetCode.append(betCode[i][betCode[i].length - 2] + " ");
				}
				if (betCode[i][betCode[i].length - 1] < 10) {
					strBetCode.append("0" + betCode[i][betCode[i].length - 1]);
				} else {
					strBetCode.append(betCode[i][betCode[i].length - 1]);
				}
		...
		case 3:
			...
			for (int j = 0; j < betCode.length; j ++) {
				...
				if (caipiaoWanfaIndex == 4) {
					...
					strBetCode.append(keyValue[betCode[0][0]]).append(keyValue[betCode[0][1]]);
					
				//其它玩法
				} else {
					for (int i = 0; i < betCode[0].length - 1; i ++) {
						strBetCode.append(betCode[0][i] + ",");
					}
					
					strBetCode.append(betCode[0][betCode[0].length - 1]);
				}
		...
		case 4:
			...
			for (int j = 0; j < betCode.length; j ++) {
				...
				for (int i = 0; i < betCode[0].length - 1; i ++) {
					strBetCode.append(get2bit(betCode[0][i]) + gap);
				}
				
				strBetCode.append(get2bit(betCode[0][betCode[0].length - 1]));
		...
		case 5:
			for (int j = 0; j < betCode.length; j ++) {
				...
				for (int i = 0; i < betCode[0].length; i ++) {
					strBetCode.append(get2bit(betCode[0][i]));
				}
				strBetCode.append("^_01_200_200");
	...
	public static String betCodeToString(int type, int maxNum, String playType,int mul, int[][] betCode) {
		...
		return betCodeToString(type, 0, maxNum, playType, mul, betCode);
	}
