...
public class BuyActivity extends Activity implements OnClickListener {
	...
	private int[] imageViews = { R.drawable.ico_buy, R.drawable.ico_double,R.drawable.ico_super,
			...
			R.drawable.icon_jc, R.drawable.ico_eleven,R.drawable.ico_expert,R.drawable.gd_eleven,
	...
	private String[] imageTitle = { "合买大厅", "双色球", "大乐透", "福彩3D", "江西11选5","时时彩", "竞彩足球","11运夺金","专家荐号", "广东11选5","排列三", "七乐彩","22选5","排列五","七星彩","足彩",
			 "竞彩篮球"};
	private final Class[] cla = { JoinInfoActivity.class, Ssq.class,Dlt.class,
			Fc3d.class,Dlc.class, Ssc.class,  ZqMainActivity.class,Eleven.class,ExpertActivity.class,GdEleven.class,PL3.class,
...
//		getBuyTodaydata();
	...
	public void initNumber() {
		...
		for (int i = 0; i <caizhongSettingList.size(); i++) {
			String caizhongSetting=(String)caizhongSettingList.get(i).get("caizhongSetting");
...
//		}
	...
	private void  getCaizhongSharePreferences() {
		//这里判断彩种设置
		RWSharedPreferences	shellRW =  new RWSharedPreferences(BuyActivity.this, ShellRWConstants.CAIZHONGSETTING);
...
//		caizhongSettingList.add(shellRW.getStringValue("hmdt").toString());//合买大厅
//		caizhongSettingList.add(shellRW.getStringValue("ssq").toString());//双色球
//		caizhongSettingList.add(shellRW.getStringValue("cjdlt").toString());//大乐透
//		caizhongSettingList.add(shellRW.getStringValue("fc3d").toString());//福彩3D
//		caizhongSettingList.add(shellRW.getStringValue("11-5").toString());//江西11-5
//		caizhongSettingList.add(shellRW.getStringValue("ssc").toString());//时时彩
//		caizhongSettingList.add(shellRW.getStringValue("jcz").toString());//竞猜足球
//		caizhongSettingList.add(shellRW.getStringValue("gd-10").toString());//广东快乐十分
//		caizhongSettingList.add(shellRW.getStringValue("gd-11-5").toString());//广东11-5
//		caizhongSettingList.add(shellRW.getStringValue("11-ydj").toString());//11运夺金
//		caizhongSettingList.add( shellRW.getStringValue("zjjh").toString());//专家荐号
//		caizhongSettingList.add(shellRW.getStringValue("pl3").toString());;//排列3
//		caizhongSettingList.add(shellRW.getStringValue("qlc").toString());//七乐彩
//		caizhongSettingList.add(shellRW.getStringValue("22-5").toString());//22-5
//		caizhongSettingList.add(shellRW.getStringValue("pl5").toString());//排列5
//		caizhongSettingList.add(shellRW.getStringValue("qxc").toString());//七星彩
//		caizhongSettingList.add(shellRW.getStringValue("zc").toString());//足彩
//		caizhongSettingList.add(shellRW.getStringValue("jcl").toString());//竞猜篮球
	}
	/**
	 * 初始化滑动
	 */
	...
	public void initImgView() {
		...
		try {
			String warning=Constants.todayjosn.getString("inProgressActivityCount");
			if(!warning.equals("0")){
				war.setText(warning);
	...
	private class GalleryViewItem extends TableLayout {
		...
		public void initBtn(View view, int position) {
			...
			for (int i = 0; i < length; i++) {
				...
				imgView.setBackgroundResource(Integer.parseInt(newList.get(index).get("imageRes")));
				String lotno=newList.get(index).get("lotno");
				if(Constants.todayjosn!=null){
				if(!lotno.equals("hmdt")&&!lotno.equals("gd-10")&&!lotno.equals("zjjh")){
					if(lotno.equals("F47104")||lotno.equals("F47102")||lotno.equals("T01001")){
						try {
							String josn=Constants.todayjosn.getString(lotno);
							JSONObject jsonobj=new JSONObject(josn);
							String isTodayOpenPrize=jsonobj.getString("isTodayOpenPrize");
							String isaddString=jsonobj.getString("isAddAward");
							if(isTodayOpenPrize.equals("true")&&isaddString.equals("true")){
								imgView.setImageResource(R.drawable.kaijiang);
							}else if(isTodayOpenPrize.equals("true")&&isaddString.equals("fasle")){
								imgView.setImageResource(R.drawable.kaijiang);
							}else if(isTodayOpenPrize.equals("false")&&isaddString.equals("true")){
				...
				imgView.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						intent.setClassName(BuyActivity.this, newList.get(index).get("className"));
						startActivity(intent);
					}
				});
				...
				if(title.length()>4){
//					textTitle.setLayoutParams(new LayoutParams(70, LayoutParams.WRAP_CONTENT));
				}
